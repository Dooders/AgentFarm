...
- **BLX-α blend crossover** (`CrossoverMode.BLEND`) — a new gene-level crossover operator that samples each offspring gene from a uniform range extended by `blend_alpha` beyond the parent interval, allowing exploration beyond the parents' convex hull.
