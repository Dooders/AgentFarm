#!/usr/bin/env python3
"""
Test script for Hydra configuration system.
This script tests the new Hydra-based configuration management.
"""
...
from farm.core.config_hydra import HydraConfigManager, create_hydra_config_manager
...
def test_basic_config_loading():
    ...
    config_manager = create_hydra_config_manager(
    ...
    print(f"Simulation config type: {type(sim_config)}")
    print(f"Max steps from sim config: {sim_config.max_steps}")
    print(f"Debug from sim config: {getattr(sim_config, 'debug', 'Not found')}")
