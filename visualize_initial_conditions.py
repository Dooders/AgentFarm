...
def visualize_simulation(iteration, experiment_path, dominance_type):
    ...
    for _, agent in agents.iterrows():
        ...
        if distances:
            ...
            nearest_pos = (nearest_resource['position_x'], nearest_resource['position_y'])
    ...
    textstr = "\n".join([
        f"Population Dominance: {pop_dom}",
        f"Survival Dominance: {surv_dom}"
    ])
...
def main():
    ...
    for dom_type, iterations in dominance_types.items():
        if len(iterations) > 5:
            selected_iterations[dom_type] = np.random.choice(iterations, 5, replace=False).tolist()
        else:
    ...
    for dom_type, iterations in selected_iterations.items():
        for iteration in iterations:
            ...
            if fig:
                fig.savefig(f"initial_conditions_viz/iteration_{iteration}_{dom_type}_dominance.png", 
        ...
        for j, iteration in enumerate(iterations):
            if j < 5:  # Ensure we don't exceed the grid
                ...
                if os.path.exists(db_path):
                    ...
                    # Add row labels
                    if j == 0:
                        axes[ax_idx].set_ylabel(f"{dom_type.capitalize()} Dominance", fontsize=12)
    
    # Add a legend to the figure
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='green', markersize=10, label='Resources'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=10, label='System Agent'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label='Independent Agent'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='orange', markersize=10, label='Control Agent')
    ]
    fig.legend(handles=legend_elements, loc='upper right', fontsize=12)
    
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # Adjust for the suptitle
    plt.savefig("initial_conditions_viz/comparative_initial_conditions.png", dpi=150, bbox_inches='tight')
    plt.close()
