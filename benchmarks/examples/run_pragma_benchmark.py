...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run pragma profile benchmark with visualization"
    )
...
def visualize_results(results):
    ...
    try:
        ...
        width = 0.8 / len(
            profiles
        )  # Dynamically calculate width based on number of profiles
        ...
        for i, profile in enumerate(profiles):
            ...
            ax1.bar(
                x + offsets[i], durations, width, label=profile, color=colors[profile]
            )
        ...
        if non_balanced_profiles:
            ...
            non_balanced_offsets = [
                width * i - half_width for i in range(len(non_balanced_profiles))
            ]

            for i, profile in enumerate(non_balanced_profiles):
                speedups = [
                    normalized_results[workload][profile] for workload in workloads
                ]
                ax2.bar(
                    x + non_balanced_offsets[i],
                    speedups,
                    width,
                    label=profile,
        ...
        width = 0.8 / len(
            workloads
        )  # Dynamically calculate width based on number of workloads
        ...
        for i, workload in enumerate(workloads):
            if workload not in workload_colors:
                workload_colors[workload] = default_workload_colors[
                    i % len(default_workload_colors)
                ]

        for i, workload in enumerate(workloads):
            durations = [raw_results[profile][workload] for profile in profiles]
            ax3.bar(
                x + workload_offsets[i],
                durations,
                width,
                label=workload,
        ...
        ax4.set_theta_offset(np.pi / 2)  # type: ignore
        ax4.set_theta_direction(-1)  # type: ignore
        ax4.set_rlabel_position(0)  # type: ignore
        ...
        params_text = f"Parameters: {results.parameters}\n" f"Generated: {timestamp}"
