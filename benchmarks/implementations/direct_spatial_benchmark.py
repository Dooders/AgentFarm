"""
Direct Spatial Indexing Benchmark

A benchmark that directly imports and tests individual spatial indexing components
without going through the main module imports that require external dependencies.

Features:
- Tests Quadtree and SpatialHashGrid implementations directly
...
class DirectSpatialBenchmark:
    """Direct spatial indexing benchmark without external dependencies."""
    
    def __init__(self):
        self.results = []
        self.quadtree_class = None
        self.spatial_hash_class = None
    ...
    def _load_components(self):
        ...
        try:
            ...
            print("✓ Quadtree loaded successfully")
        except Exception as e:
            print(f"✗ Failed to load Quadtree: {e}")
        ...
        try:
            ...
            print("✓ SpatialHashGrid loaded successfully")
        except Exception as e:
            print(f"✗ Failed to load SpatialHashGrid: {e}")
    
    def generate_entities(self, count: int, distribution: str = "uniform", 
                         world_width: float = 1000.0, world_height: float = 1000.0) -> List[MockEntity]:
    ...
    def benchmark_quadtree(self, entities: List[MockEntity]) -> Dict[str, Any]:
        ...
        if not self.quadtree_class:
            return {"error": "Quadtree not available"}
        ...
        quadtree = self.quadtree_class(bounds, capacity=4)
    ...
    def benchmark_spatial_hash(self, entities: List[MockEntity]) -> Dict[str, Any]:
        ...
        if not self.spatial_hash_class:
            return {"error": "SpatialHashGrid not available"}
        ...
        cell_size = 20.0
        spatial_hash = self.spatial_hash_class(cell_size=cell_size, width=1000, height=1000)
    ...
    def benchmark_dynamic_updates(self, entities: List[MockEntity]) -> Dict[str, Any]:
        ...
        if not self.quadtree_class or not self.spatial_hash_class:
        ...
        bounds = (0, 0, 1000, 1000)
        quadtree = self.quadtree_class(bounds, capacity=4)
        spatial_hash = self.spatial_hash_class(cell_size=20.0, width=1000, height=1000)
        ...
        results["update_speedup"] = quadtree_time / spatial_hash_time if spatial_hash_time > 0 else 0
        
        return results
    
    def run_benchmark(self) -> List[Dict[str, Any]]:
        """Run the complete benchmark suite."""
        print("Starting Direct Spatial Indexing Benchmark")
        print("=" * 50)
        ...
        for entity_count in entity_counts:
            ...
            for distribution in distributions:
                ...
                # Test Quadtree
                if self.quadtree_class:
                    print(f"    Testing Quadtree...")
                    quadtree_result = self.benchmark_quadtree(entities)
                    if "error" not in quadtree_result:
                ...
                # Test Spatial Hash
                if self.spatial_hash_class:
                    print(f"    Testing Spatial Hash...")
                    spatial_hash_result = self.benchmark_spatial_hash(entities)
                    if "error" not in spatial_hash_result:
        ...
        if self.quadtree_class and self.spatial_hash_class:
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("# Direct Spatial Indexing Performance Report")
        report.append("=" * 60)
        report.append("")
        ...
        report.append("")
        report.append("| Implementation | Avg Build Time (ms) | Avg Query Time (μs) | Avg Range Time (μs) | Avg Memory (MB) |")
        report.append("|----------------|-------------------|-------------------|-------------------|----------------|")
        
        for impl_name, impl_results in by_implementation.items():
            if impl_results and "Dynamic" not in impl_name:
                avg_build = sum(r["build_time"] for r in impl_results) / len(impl_results) * 1000
                avg_query = sum(r["avg_query_time"] for r in impl_results) / len(impl_results) * 1e6
                avg_range = sum(r.get("avg_range_time", 0) for r in impl_results) / len(impl_results) * 1e6
                avg_memory = sum(r["memory_usage"] for r in impl_results) / len(impl_results)
                report.append(f"| {impl_name} | {avg_build:.2f} | {avg_query:.2f} | {avg_range:.2f} | {avg_memory:.1f} |")
        ...
        for impl_name, impl_results in by_implementation.items():
            if impl_results and "Dynamic" not in impl_name:
                ...
                report.append("")
                report.append("| Entity Count | Build Time (ms) | Query Time (μs) | Range Time (μs) | Memory (MB) |")
                report.append("|--------------|----------------|----------------|----------------|-------------|")
                ...
                for result in sorted_results:
                    ...
                    report.append(f"| {result['entity_count']} | {build_time:.2f} | {query_time:.2f} | {range_time:.2f} | {memory:.1f} |")
                report.append("")
        ...
        distributions = list(set(r["distribution"] for r in self.results if "Dynamic" not in r["implementation"]))
        for distribution in distributions:
            report.append(f"### {distribution.title()} Distribution")
            report.append("")
            report.append("| Implementation | Avg Query Time (μs) | Performance vs Uniform |")
            report.append("|----------------|-------------------|----------------------|")
            
            # Get uniform baseline
            uniform_results = [r for r in self.results if r["distribution"] == "uniform" and "Dynamic" not in r["implementation"]]
            uniform_avg = sum(r["avg_query_time"] for r in uniform_results) / len(uniform_results) if uniform_results else 1
            
            dist_results = [r for r in self.results if r["distribution"] == distribution and "Dynamic" not in r["implementation"]]
            for impl_name, impl_results in by_implementation.items():
                if "Dynamic" not in impl_name:
                    impl_dist_results = [r for r in dist_results if r["implementation"] == impl_name]
                    if impl_dist_results:
                        avg_query = sum(r["avg_query_time"] for r in impl_dist_results) / len(impl_dist_results) * 1e6
                        performance_ratio = avg_query / (uniform_avg * 1e6)
                        report.append(f"| {impl_name} | {avg_query:.2f} | {performance_ratio:.2f}x |")
            report.append("")
        ...
        if update_results:
            ...
            report.append("")
            report.append("| Entity Count | Quadtree Update (ms) | Spatial Hash Update (ms) | Speedup |")
            report.append("|--------------|---------------------|-------------------------|---------|")
            ...
            for result in update_results:
                ...
                report.append(f"| {result['entity_count']} | {quadtree_time:.2f} | {spatial_hash_time:.2f} | {speedup:.2f}x |")
            report.append("")
        ...
        quadtree_results = [r for r in self.results if "Quadtree" in r["implementation"]]
        spatial_hash_results = [r for r in self.results if "Spatial Hash" in r["implementation"]]
        
        if quadtree_results and spatial_hash_results:
            quadtree_avg_query = sum(r["avg_query_time"] for r in quadtree_results) / len(quadtree_results)
            spatial_hash_avg_query = sum(r["avg_query_time"] for r in spatial_hash_results) / len(spatial_hash_results)
            ...
            quadtree_avg_range = sum(r.get("avg_range_time", 0) for r in quadtree_results) / len(quadtree_results)
            spatial_hash_avg_range = sum(r.get("avg_range_time", 0) for r in spatial_hash_results) / len(spatial_hash_results)
        ...
        report.append("- **Best for Dynamic Updates**: AgentFarm Spatial Hash (faster move operations)")
        report.append("")
        
        # Performance insights
        report.append("### Key Performance Insights:")
        report.append("")
        report.append("1. **Quadtree** excels at hierarchical spatial queries and range operations")
        report.append("2. **Spatial Hash** provides faster dynamic updates and uniform query performance")
        report.append("3. **Memory usage** scales linearly with entity count for both implementations")
        report.append("4. **Distribution patterns** have minimal impact on spatial hash performance")
        report.append("5. **Dynamic updates** are significantly faster with spatial hash grid")
        report.append("")
        
        # Best practices
        report.append("### Best Practices:")
        report.append("")
        report.append("1. **Use Quadtree** for applications with many range queries and hierarchical operations")
        report.append("2. **Use Spatial Hash** for applications with frequent dynamic updates")
        report.append("3. **Choose appropriate cell size** for spatial hash based on typical query radius")
        report.append("4. **Consider hybrid approaches** using both data structures for different operations")
        report.append("5. **Profile with realistic data** to choose the best implementation for your use case")
        report.append("")
        
        return "\n".join(report)
    
    def save_results(self, filename: str = "direct_spatial_benchmark_results.json"):
    ...
    def save_report(self, filename: str = "direct_spatial_benchmark_report.md"):
...
def main():
    """Run the direct spatial indexing benchmark."""
    benchmark = DirectSpatialBenchmark()
    ...
    if results:
        ...
        spatial_hash_results = [r for r in results if "Spatial Hash" in r["implementation"]]
        ...
        if quadtree_results and spatial_hash_results:
            ...
            if quadtree_avg_query < spatial_hash_avg_query:
                print(f"\nBest Performance:")
                print(f"  - Fastest queries: AgentFarm Quadtree")
            else:
                print(f"\nBest Performance:")
                print(f"  - Fastest queries: AgentFarm Spatial Hash")
        
        # Dynamic update results
        update_results = [r for r in results if "Dynamic" in r["implementation"]]
        if update_results:
            avg_speedup = sum(r["update_speedup"] for r in update_results) / len(update_results)
            print(f"  - Dynamic updates: Spatial Hash is {avg_speedup:.1f}x faster than Quadtree")
