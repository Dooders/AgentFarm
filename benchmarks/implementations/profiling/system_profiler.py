...
class SystemProfiler:
    ...
    def profile_scaling_agents(self, agent_counts: List[int], steps: int = 100):
        ...
        for num_agents in agent_counts:
            ...
            try:
                ...
                memory_growth = (
                    after_metrics["memory_rss_mb"] - before_metrics["memory_rss_mb"]
                )
                ...
                results[num_agents] = {
                    ...
                    "memory_per_agent_kb": (
                        (memory_growth * 1024) / num_agents if num_agents > 0 else 0
                    ),
                ...
                print(
                    f"  Memory: {memory_growth:+.1f} MB ({memory_growth*1024/num_agents:.1f} KB/agent)"
                )
    ...
    def profile_scaling_environment(
        self, env_sizes: List[Tuple[int, int]], num_agents: int = 50, steps: int = 100
    ):
        """Profile how performance scales with environment size."""
        print("\n" + "=" * 60)
        print(
            f"Profiling Environment Size Scaling ({num_agents} agents, {steps} steps)"
        )
        print("=" * 60 + "\n")
    ...
    def profile_memory_over_time(
        self, num_agents: int = 50, steps: int = 500, sample_interval: int = 10
    ):
        ...
        try:
            ...
            results = {
                ...
                "memory_growth_mb": after_metrics["memory_rss_mb"]
                - before_metrics["memory_rss_mb"],
            ...
            print(
                f"  Memory: {before_metrics['memory_rss_mb']:.1f} MB → "
    ...
    def generate_report(self):
        ...
        if "scaling_agents" in self.results:
            ...
            for num_agents, data in sorted(self.results["scaling_agents"].items()):
                if "error" not in data:
                    print(
                        f"  {num_agents:>4} agents: {data['steps_per_second']:>6.1f} steps/s, "
            ...
            scaling_data = [
                (k, v)
                for k, v in self.results["scaling_agents"].items()
                if "error" not in v
            ]
        ...
        if "scaling_steps" in self.results:
            ...
            for num_steps, data in sorted(self.results["scaling_steps"].items()):
                if "error" not in data:
                    print(
                        f"  {num_steps:>5} steps: {data['steps_per_second']:>6.1f} steps/s, "
