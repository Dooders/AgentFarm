...
class MemoryDBBenchmark(Benchmark):
    ...
    def setup(self) -> None:
        ...
        self.config.control_agents = num_agents - (2 * (num_agents // 3))  # Ensure total is num_agents
    ...
    def run(self) -> Dict[str, Any]:
        ...
        self.config.persist_in_memory_db = False
        ...
        print("Running with in-memory database (with persistence) - RECOMMENDED FOR PRODUCTION...")
        ...
        self.config.use_in_memory_db = True
        self.config.persist_in_memory_db = True
        ...
        print(f"In-memory database with persistence time: {memory_persist_time:.2f} seconds")
        print("NOTE: This configuration is recommended for production use when post-simulation analysis is required.")
        
        # Calculate speedups
        disk_to_memory_speedup = disk_time / memory_time if memory_time > 0 else float('inf')
        disk_to_memory_persist_speedup = disk_time / memory_persist_time if memory_persist_time > 0 else float('inf')
        
        print(f"Speedup (disk to memory): {disk_to_memory_speedup:.2f}x")
        print(f"Speedup (disk to memory with persistence): {disk_to_memory_persist_speedup:.2f}x")
