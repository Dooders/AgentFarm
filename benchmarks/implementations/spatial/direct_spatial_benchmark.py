...
class DirectSpatialBenchmark:
    ...
    def _load_components(self):
        ...
        if current_dir.endswith("/workspace"):
            ...
            workspace_root = current_dir
            while workspace_root != "/" and not os.path.exists(
                os.path.join(workspace_root, "farm")
            ):
                workspace_root = os.path.dirname(workspace_root)

        quadtree_path = os.path.join(
            workspace_root, "farm", "core", "spatial", "quadtree.py"
        )
        hash_grid_path = os.path.join(
            workspace_root, "farm", "core", "spatial", "hash_grid.py"
        )
    ...
    def benchmark_spatial_hash(self, entities: List[MockEntity]) -> Dict[str, Any]:
        ...
        cell_size = 20.0
        spatial_hash = self.spatial_hash_class(
            cell_size=cell_size, width=1000, height=1000
        )
