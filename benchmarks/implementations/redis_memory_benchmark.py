...
from farm.memory.redis_memory import AgentMemory, RedisMemoryConfig, AgentMemoryManager
from farm.agents.agent_state import AgentState
from farm.simulation.perception import PerceptionData
...
class RedisMemoryBenchmark(Benchmark):
    ...
    def setup(self) -> None:
        ...
        self.memories = [self.memory_manager.get_memory(agent_id) for agent_id in self.agent_ids]
    ...
    def _generate_test_data(self) -> None:
        ...
        for step in range(num_entries):
            # Create a test state
            state = AgentState()
            state.position = (random.uniform(0, 100), random.uniform(0, 100))
            state.orientation = random.uniform(0, 360)
            state.energy = random.uniform(0, 100)
            state.health = random.uniform(0, 100)
            
            # Create test perception data
            perception = PerceptionData()
            perception.visible_agents = random.randint(0, 5)
            perception.visible_resources = random.randint(0, 10)
            ...
            # Create test action
            actions = ["move", "eat", "sleep", "attack", "defend", "gather", "build", "communicate"]
    ...
    def _benchmark_writes(self) -> Dict[str, Any]:
        ...
        percentiles = {
            ...
            "p99": statistics.quantiles(durations, n=100)[98] if len(durations) >= 100 else None
        }
    ...
    def _benchmark_reads(self) -> Dict[str, Any]:
        ...
        for _ in range(min(10, len(self.test_data))):
            ...
            memory.get_memories_by_step(step)
        
        # Create a random sample of steps to read
        steps_to_read = [data["step"] for data in random.sample(self.test_data, len(self.test_data))]
        
        # Benchmark
        start_time = time.time()
        for step in steps_to_read:
            before = time.time()
            memories = memory.get_memories_by_step(step)
            after = time.time()
            durations.append(after - before)
        end_time = time.time()
    ...
    def _benchmark_searches(self) -> Dict[str, Any]:
        ...
        positions = [(random.uniform(0, 100), random.uniform(0, 100)) for _ in range(100)]
    ...
    def _benchmark_memory_usage(self) -> Dict[str, Any]:
        ...
        for i in range(0, len(self.test_data), batch_size):
            ...
            memory_per_entry = (current_used_memory - initial_used_memory) / max(1, entries_count)
        ...
        print(f"Memory usage: {total_memory_used} bytes total, {bytes_per_entry:.2f} bytes/entry")
