...
from farm.database import SimulationDatabase, InMemorySimulationDatabase
...
class PragmaProfileBenchmark(Benchmark):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for profile in self.profiles:
            ...
            for workload in self.workloads:
                ...
                print(f"  {profile} profile, {workload} workload: {duration:.2f} seconds")
        ...
        for workload in self.workloads:
            ...
            for profile in self.profiles:
                ...
                speedup = baseline / results[profile][workload] if results[profile][workload] > 0 else float('inf')
                normalized_results[workload][profile] = speedup
                
                # Print speedup (>1 means faster, <1 means slower)
                print(f"  {profile} profile is {speedup:.2f}x {'faster' if speedup > 1 else 'slower'} than balanced for {workload} workload")
    ...
    def _test_write_performance(self, profile: str) -> float:
        ...
        db_path = os.path.join(self.temp_dir, f"{profile}_write_test_{unique_suffix}.db")
        ...
        cursor.execute(f"CREATE TABLE {table_name} (id INTEGER PRIMARY KEY, data TEXT, value REAL, timestamp INTEGER)")
    ...
    def _prepare_read_test_database(self, db_path: str, profile: str, table_name: str) -> None:
        """
        ...
        while current_size_mb < db_size_mb:
            ...
            records = []
            for j in range(batch_size):
                data = f"data_{record_id}" * 100  # Make the data larger to reach target size faster
                value = record_id / 1000.0
                timestamp = int(time.time())
                records.append((record_id, data, value, timestamp))
                record_id += 1
            ...
            if record_id % 10000 == 0:
                print(f"  Prepared {record_id} records, current size: {current_size_mb:.2f} MB")
    ...
    def _test_mixed_performance(self, profile: str) -> float:
        ...
        # Create a database file for this test with a unique name
        unique_suffix = f"{self.run_id}_{int(time.time())}"
        db_path = os.path.join(self.temp_dir, f"{profile}_mixed_test_{unique_suffix}.db")
        ...
        num_operations = self.parameters["num_records"] // 10  # Fewer operations for mixed workload
        ...
        for i in range(num_operations):
            ...
            if i % 10 == 0:
                ...
                end_id = i
                cursor.execute(f"SELECT COUNT(*) FROM {table_name} WHERE id BETWEEN ? AND ?", (start_id, end_id))
                count = cursor.fetchone()[0]
                
                # Aggregation
                cursor.execute(f"SELECT AVG(value) FROM {table_name} WHERE id <= ?", (i,))
    ...
    def cleanup(self) -> None:
        ...
        if self.temp_dir and os.path.exists(self.temp_dir):
            try:
                ...
                print("This is not critical, but you may want to clean it up manually later.") 