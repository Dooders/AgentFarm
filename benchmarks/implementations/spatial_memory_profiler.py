...
class MemorySnapshot:
    ...
    def get_peak_memory_mb(self) -> float:
        ...
        return self.memory_info.peak_wss / 1024 / 1024 if hasattr(self.memory_info, 'peak_wss') else self.get_memory_mb()
...
class SpatialMemoryProfiler:
    ...
    def analyze_memory_growth(self) -> Dict[str, Any]:
        ...
        analysis["total_growth_mb"] = last_snapshot.get_memory_mb() - first_snapshot.get_memory_mb()
        
        # Find peak usage
        analysis["peak_usage_mb"] = max(snapshot.get_memory_mb() for snapshot in self.snapshots)
        ...
        if len(memory_values) > 3:
            ...
            analysis["memory_leaks_detected"] = growth_periods > len(memory_values) * 0.7
...
class SpatialMemoryBenchmark:
    ...
    def profile_agentfarm_spatial_index(
        ...
        build_memory = after_build_snapshot.get_memory_mb() - after_registration_snapshot.get_memory_mb()
        results["memory_per_entity_kb"] = (build_memory * 1024) / len(entities)
        
        # Calculate query memory overhead
        query_memory = after_queries_snapshot.get_memory_mb() - after_build_snapshot.get_memory_mb()
        results["query_memory_overhead_mb"] = query_memory
        
        # Calculate update memory overhead
        update_memory = after_updates_snapshot.get_memory_mb() - after_queries_snapshot.get_memory_mb()
        results["update_memory_overhead_mb"] = update_memory
        
        # GC impact
        gc_memory = after_gc_snapshot.get_memory_mb() - after_updates_snapshot.get_memory_mb()
    ...
    def profile_scipy_kdtree(self, entities: List[MockEntity]) -> Dict[str, Any]:
        ...
        # Calculate memory per entity
        build_memory = after_build_snapshot.get_memory_mb() - after_positions_snapshot.get_memory_mb()
        results["memory_per_entity_kb"] = (build_memory * 1024) / len(entities)
        ...
        # GC impact
        gc_memory = after_gc_snapshot.get_memory_mb() - after_queries_snapshot.get_memory_mb()
        results["gc_impact_mb"] = gc_memory
    ...
    def profile_memory_scaling(self) -> Dict[str, Any]:
        ...
        for count in entity_counts:
            ...
            implementations = [
                ("agentfarm_kdtree", lambda: self.profile_agentfarm_spatial_index(entities, "kdtree")),
                ("agentfarm_quadtree", lambda: self.profile_agentfarm_spatial_index(entities, "quadtree")),
                ("agentfarm_spatial_hash", lambda: self.profile_agentfarm_spatial_index(entities, "spatial_hash")),
                ("scipy_kdtree", lambda: self.profile_scipy_kdtree(entities)),
            ]
            ...
            for impl_name, impl_func in implementations:
                ...
                scaling_results[impl_name].append({
                    ...
                    "peak_memory_mb": result["memory_analysis"].get("peak_usage_mb", 0),
        ...
        if implementations:
            ...
            for impl_name, impl_results in results.items():
                ...
                if len(impl_results) > 1:
                    ...
                    scaling_factor = last_memory / first_memory if first_memory > 0 else 1
                    
                    analysis["scaling_efficiency"][impl_name] = {
                        "scaling_factor": scaling_factor,
                        "is_linear": abs(scaling_factor - 1.0) < 0.1,
                        "memory_growth_rate": (last_memory - first_memory) / len(impl_results),
                    }
    ...
    def generate_memory_report(self, results: Dict[str, Any]) -> str:
        ...
        if analysis["efficiency_ranking"]:
            ...
            report.append(f"**Most Memory-Efficient Implementation**: {most_efficient[0]}")
        ...
        for i, (impl_name, memory_per_entity) in enumerate(analysis["efficiency_ranking"], 1):
        ...
        report.append("")
        report.append("| Implementation | Scaling Factor | Linear Scaling | Growth Rate (KB/step) |")
        report.append("|----------------|----------------|----------------|----------------------|")
        
        for impl_name, scaling_data in analysis["scaling_efficiency"].items():
            linear = "✓" if scaling_data["is_linear"] else "✗"
            report.append(f"| {impl_name} | {scaling_data['scaling_factor']:.2f} | {linear} | {scaling_data['memory_growth_rate']:.2f} |")
        ...
        for impl_name, impl_results in results.items():
            ...
            if impl_results:
                ...
                report.append("")
                report.append("| Entity Count | Memory per Entity (KB) | Total Memory (MB) | Peak Memory (MB) | GC Impact (MB) |")
                report.append("|--------------|------------------------|-------------------|------------------|----------------|")
                
                for result in impl_results:
                    report.append(f"| {result['entity_count']} | {result['memory_per_entity_kb']:.2f} | {result['total_memory_mb']:.1f} | {result['peak_memory_mb']:.1f} | {result['gc_impact_mb']:.1f} |")
        ...
        report.append("")
        report.append("1. **Memory per entity** varies significantly between implementations")
        report.append("2. **Scaling behavior** shows which implementations maintain efficiency at scale")
        report.append("3. **GC impact** indicates memory fragmentation and cleanup efficiency")
        report.append("4. **Peak memory usage** helps determine memory requirements for production")
        report.append("")
        
        # Best practices
        report.append("### Memory Optimization Best Practices:")
        report.append("")
        report.append("1. **Choose efficient implementations** for memory-constrained environments")
