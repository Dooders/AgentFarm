...
from benchmarks.implementations.comprehensive_spatial_benchmark import SpatialBenchmark, SpatialBenchmarkConfig
from benchmarks.implementations.spatial_memory_profiler import SpatialMemoryBenchmark
from benchmarks.implementations.spatial_performance_analyzer import SpatialPerformanceAnalyzer
...
def run_memory_profiling():
    ...
    # Save results
    import json
    with open("/workspace/benchmarks/results/spatial_memory_scaling.json", "w") as f:
        json.dump(scaling_results, f, indent=2)
    
    # Generate and save report
    report = benchmark.generate_memory_report(scaling_results)
    with open("/workspace/benchmarks/results/spatial_memory_report.md", "w") as f:
        f.write(report)
...
def generate_visualizations(comprehensive_results=None, memory_results=None):
    ...
    if comprehensive_results is None:
        try:
            comprehensive_results = visualizer.load_benchmark_results("comprehensive_spatial_benchmark.json")
    ...
    if comprehensive_results.get("results"):
        ...
        visualizer.create_performance_comparison_chart(comprehensive_results, "build_time", "Build Time Comparison")
        visualizer.create_performance_comparison_chart(comprehensive_results, "avg_query_time", "Query Time Comparison")
        
        # Memory and scaling charts
        visualizer.create_memory_usage_chart(comprehensive_results)
        visualizer.create_scaling_analysis_chart(comprehensive_results)
        visualizer.create_distribution_impact_chart(comprehensive_results)
        visualizer.create_performance_heatmap(comprehensive_results)
        
        # Comprehensive dashboard
        visualizer.create_comprehensive_dashboard(comprehensive_results, memory_results)
    ...
    report = visualizer.generate_visualization_report(comprehensive_results, memory_results)
...
def run_performance_analysis(comprehensive_results=None, memory_results=None):
    ...
    if comprehensive_results is None:
        try:
            comprehensive_results = analyzer.load_benchmark_results("comprehensive_spatial_benchmark.json")
        except FileNotFoundError:
    ...
    report = analyzer.generate_comprehensive_report(comprehensive_results, memory_results)
...
def create_summary_report():
    ...
    try:
        ...
        import json
        with open("/workspace/benchmarks/results/comprehensive_spatial_benchmark.json", "r") as f:
            results = json.load(f)
        
        if results.get("results"):
            implementations = list(set(r["implementation"] for r in results["results"]))
            summary.append(f"- **Total Implementations Tested**: {len(implementations)}")
            ...
            # Best build time
            best_build = min(by_impl.keys(), key=lambda impl: 
                sum(r["build_time"] for r in by_impl[impl]) / len(by_impl[impl]))
            summary.append(f"- **Fastest Build Time**: {best_build}")
            
            # Best query time
            best_query = min(by_impl.keys(), key=lambda impl: 
                sum(r["avg_query_time"] for r in by_impl[impl]) / len(by_impl[impl]))
            summary.append(f"- **Fastest Query Time**: {best_query}")
            
            # Best memory usage
            best_memory = min(by_impl.keys(), key=lambda impl: 
                sum(r["memory_usage"] for r in by_impl[impl]) / len(by_impl[impl]))
    ...
    summary.append("")
    summary.append("1. **For Real-Time Applications**: Use implementations with fastest query times")
    summary.append("2. **For Memory-Constrained Systems**: Choose implementations with lowest memory usage")
    summary.append("3. **For Dynamic Simulations**: Use AgentFarm implementations with batch update support")
    summary.append("4. **For Large-Scale Systems**: Focus on implementations with best scaling characteristics")
    summary.append("")
    ...
    summary.append("4. Consider running additional benchmarks for your specific scenarios")
    summary.append("")
    
    # Save summary
    summary_text = "\n".join(summary)
    with open("/workspace/benchmarks/results/benchmark_summary.md", "w") as f:
...
def main():
    ...
    parser.add_argument("--memory-only", action="store_true", help="Run only memory profiling")
    parser.add_argument("--visualize-only", action="store_true", help="Generate only visualizations")
    parser.add_argument("--analyze-only", action="store_true", help="Run only performance analysis")
    ...
    try:
        ...
        print(f"Results saved to: /workspace/benchmarks/results/")
        print("")
