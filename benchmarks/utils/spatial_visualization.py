...
class SpatialBenchmarkVisualizer:
    ...
    def create_performance_comparison_chart(
        ...
        entity_counts = sorted(set(r["entity_count"] for r in results.get("results", [])))
        ...
        for impl_name, impl_results in implementations.items():
            ...
            for count in entity_counts:
                matching_results = [r for r in impl_results if r["entity_count"] == count]
                if matching_results:
            ...
            # Plot line
            ax.plot(entity_counts, metric_values, marker='o', linewidth=2, 
                   label=impl_name, markersize=8)
    ...
    def create_memory_usage_chart(self, results: Dict[str, Any]) -> plt.Figure:
        ...
        for impl_name, impl_results in implementations.items():
            ...
            ax1.plot(entity_counts, memory_values, marker='o', linewidth=2, 
                    label=impl_name, markersize=8)
        ...
        for impl_name, impl_results in implementations.items():
            ...
            ax2.plot(entity_counts, memory_per_entity, marker='s', linewidth=2, 
                    label=impl_name, markersize=8)
    ...
    def create_scaling_analysis_chart(self, results: Dict[str, Any]) -> plt.Figure:
        ...
        for impl_name, impl_results in implementations.items():
            ...
            ax1.plot(entity_counts, build_times, marker='o', linewidth=2, 
                    label=impl_name, markersize=8)
        ...
        for impl_name, impl_results in implementations.items():
            ...
            ax2.plot(entity_counts, query_times, marker='s', linewidth=2, 
                    label=impl_name, markersize=8)
        ...
        for impl_name, impl_results in implementations.items():
            ...
            ax3.plot(entity_counts, memory_usage, marker='^', linewidth=2, 
                    label=impl_name, markersize=8)
        ...
        for impl_name, impl_results in implementations.items():
            ...
            for count in entity_counts:
                matching_results = [r for r in impl_results if r["entity_count"] == count]
                if matching_results:
                    avg_query_time = np.mean([r["avg_query_time"] for r in matching_results])
                    performance_per_entity.append(avg_query_time * count)  # Total query time
                else:
                    performance_per_entity.append(np.nan)
            
            ax4.plot(entity_counts, performance_per_entity, marker='d', linewidth=2, 
                    label=impl_name, markersize=8)
    ...
    def create_distribution_impact_chart(self, results: Dict[str, Any]) -> plt.Figure:
        ...
        implementations = list(set(r["implementation"] for r in results.get("results", [])))
        ...
        ax1.set_title("Query Performance by Distribution", fontsize=14, fontweight='bold')
    ...
    def create_memory_timeline_chart(self, memory_results: Dict[str, Any]) -> plt.Figure:
        ...
        for impl_name, impl_results in memory_results.items():
            ...
            if impl_results:
                ...
                ax1.plot(entity_counts, memory_per_entity, marker='o', linewidth=2, 
                        label=impl_name, markersize=8)
        ...
        for impl_name, impl_results in memory_results.items():
            ...
            if impl_results:
                ...
                ax2.plot(entity_counts, total_memory, marker='s', linewidth=2, 
                        label=impl_name, markersize=8)
    ...
    def create_performance_heatmap(self, results: Dict[str, Any]) -> plt.Figure:
        ...
        for i, impl_name in enumerate(implementations):
            for j, count in enumerate(entity_counts):
                matching_results = [r for r in results.get("results", []) 
                                  if r["implementation"] == impl_name and r["entity_count"] == count]
                if matching_results:
                    build_time_matrix[i, j] = np.mean([r["build_time"] for r in matching_results])
        ...
        for i, impl_name in enumerate(implementations):
            for j, count in enumerate(entity_counts):
                matching_results = [r for r in results.get("results", []) 
                                  if r["implementation"] == impl_name and r["entity_count"] == count]
                if matching_results:
                    query_time_matrix[i, j] = np.mean([r["avg_query_time"] for r in matching_results])
    ...
    def create_comprehensive_dashboard(self, results: Dict[str, Any], memory_results: Dict[str, Any] = None) -> plt.Figure:
        ...
        self._plot_performance_comparison(ax1, results, "build_time", "Build Time Comparison")
        ...
        self._plot_performance_comparison(ax3, results, "avg_query_time", "Query Time Scaling")
        ...
        plt.suptitle("Spatial Indexing Performance Dashboard", fontsize=16, fontweight='bold')
        self.figures.append(fig)
        return fig
    
    def _plot_performance_comparison(self, ax, results: Dict[str, Any], metric: str, title: str):
    ...
    def _plot_memory_usage(self, ax, results: Dict[str, Any]):
        ...
        for impl_name, impl_results in implementations.items():
            ...
            ax.plot(entity_counts, memory_values, marker='s', linewidth=2, 
                   label=impl_name, markersize=6)
    ...
    def _plot_memory_efficiency(self, ax, memory_results: Dict[str, Any]):
        ...
        for impl_name, impl_results in memory_results.items():
            ...
            if impl_results:
                ...
                ax.plot(entity_counts, memory_per_entity, marker='o', linewidth=2, 
                       label=impl_name, markersize=6)
    ...
    def generate_visualization_report(self, results: Dict[str, Any], memory_results: Dict[str, Any] = None) -> str:
        ...
        report.append("")
        report.append("The following visualizations have been generated to analyze spatial indexing performance:")
        report.append("")
        ...
        if implementations:
            ...
            best_build_time = min(implementations, key=lambda impl: 
                np.mean([r["build_time"] for r in results.get("results", []) 
                        if r["implementation"] == impl]))
            
            best_query_time = min(implementations, key=lambda impl: 
                np.mean([r["avg_query_time"] for r in results.get("results", []) 
                        if r["implementation"] == impl]))
            
            best_memory = min(implementations, key=lambda impl: 
                np.mean([r["memory_usage"] for r in results.get("results", []) 
                        if r["implementation"] == impl]))
        ...
        report.append("- Distribution patterns have minimal impact on most implementations")
        report.append("")
        
        # Recommendations
        report.append("### Visualization-Based Recommendations:")
        report.append("")
        report.append("1. **For High-Frequency Queries**: Use implementations with lowest query time")
        report.append("2. **For Memory-Constrained Environments**: Choose implementations with lowest memory usage")
        report.append("3. **For Dynamic Updates**: Consider batch update capabilities")
        report.append("4. **For Large-Scale Simulations**: Focus on implementations with best scaling characteristics")
        report.append("")
...
def main():
    ...
    try:
        results = visualizer.load_benchmark_results("comprehensive_spatial_benchmark.json")
    ...
    # Load memory results
    try:
        memory_results = visualizer.load_benchmark_results("spatial_memory_scaling.json")
    ...
    if results.get("results"):
        ...
        visualizer.create_performance_comparison_chart(results, "build_time", "Build Time Comparison")
        visualizer.create_performance_comparison_chart(results, "avg_query_time", "Query Time Comparison")
