...
def calculate_statistics(results: BenchmarkResults) -> Dict[str, float]:
    ...
    if len(durations) >= 4:
        ...
        q1 = np.percentile(sorted_durations, 25)
        q3 = np.percentile(sorted_durations, 75)
        iqr = q3 - q1
    else:
        iqr = 0.0
...
def compare_statistics(results1: BenchmarkResults, results2: BenchmarkResults) -> Dict[str, float]:
    """
    ...
    for key in stats1:
        if stats1[key] == 0 and stats2[key] == 0:
            ...
            comparison[f"{key}_diff_pct"] = ((stats2[key] - stats1[key]) / stats1[key]) * 100
...
def analyze_parameter_impact(results_dict: Dict[Any, BenchmarkResults], 
                            metric: str = "mean_duration") -> Dict[str, Any]:
    """
    ...
    improvement_pct = ((max_value - min_value) / max_value) * 100 if max_value > 0 else 0.0
