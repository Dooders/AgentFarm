...
class TestAgentFarmController:
    ...
    def test_init_with_default_workspace(self, temp_workspace):
        ...
        with patch("farm.api.unified_controller.Path") as mock_path_class:
            # Create a mock path object that supports path operations
            mock_path = Mock()
            mock_path.__truediv__ = Mock(return_value=temp_workspace / "sessions")
            ...
            assert controller.workspace_path == mock_path
    ...
    def test_create_simulation_success(
        self, mock_adapter_class, temp_workspace, sample_simulation_config
    ):
        ...
        simulation_id = controller.create_simulation(
            session_id, sample_simulation_config
        )
    ...
    def test_create_simulation_nonexistent_session(
        self, temp_workspace, sample_simulation_config
    ):
        ...
        with pytest.raises(ValueError, match="Session .* not found"):
            controller.create_simulation(
                "nonexistent-session", sample_simulation_config
            )
    ...
    def test_create_experiment_success(
        self, mock_adapter_class, temp_workspace, sample_experiment_config
    ):
    ...
    def test_create_experiment_nonexistent_session(
        ...
        with pytest.raises(ValueError, match="Session .* not found"):
            controller.create_experiment(
                "nonexistent-session", sample_experiment_config
            )
    ...
    def test_analyze_simulation_success(self, mock_adapter_class, temp_workspace):
        ...
        mock_results = AnalysisResults(
            analysis_id="analysis-123", analysis_type="fitness_analysis"
        )
    ...
    def test_compare_simulations_success(self, mock_adapter_class, temp_workspace):
        ...
        mock_results = ComparisonResults(
            comparison_id="comp-123", simulation_ids=["sim-1", "sim-2"]
        )
    ...
    def test_generate_visualization_success(self, temp_workspace):
        ...
        viz_path = controller.generate_visualization(
            session_id, "sim-123", "fitness_chart"
        )
    ...
    def test_generate_visualization_nonexistent_session(self, temp_workspace):
        ...
        with pytest.raises(ValueError, match="Session .* not found"):
            controller.generate_visualization(
                "nonexistent-session", "sim-123", "fitness_chart"
            )
    ...
    def test_subscribe_to_events_success(self, mock_adapter_class, temp_workspace):
        ...
        mock_adapter.subscribe_to_events.assert_called_once_with(
            ["simulation_started", "simulation_completed"], "sim-123", None
        )
    ...
    @patch("farm.api.unified_controller.logger")
    def test_logging_in_initialization(self, mock_logger, temp_workspace):
    ...
    @patch("farm.api.unified_controller.logger")
    def test_logging_in_cleanup(self, mock_logger, temp_workspace):
