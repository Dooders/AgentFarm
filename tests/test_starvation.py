#!/usr/bin/env python3
"""Test agent starvation and pool release mechanism"""
...
def test_starvation():
    print("Testing agent starvation and pool release...")

    env = MockEnvironment()

    # Create initial agents with low resources
    print("Creating 5 initial agents with resource_level=1:")
    for i in range(5):
    ...
    step = 0
    while len(env.agents) > 0 and step < 10:
        print(f"\nStep {step}:")
        ...
        # Stop if no more agents are dying
