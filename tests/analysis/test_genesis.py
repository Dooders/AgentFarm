...
class TestGenesisComputations:
    ...
    def test_compute_initial_state_metrics(self, mock_advantages, mock_attributes, 
                                          mock_agent_proximity, mock_resource_proximity, 
                                          mock_session, mock_agents, mock_resources):
        """Test initial state metrics computation."""
        # Mock query results
        mock_session.query.return_value.filter.return_value.all.return_value = [
            (a['agent_id'], a['agent_type'], a['position_x'], a['position_y'],
             a['initial_resources'], a['starting_health'], a['starvation_counter'],
             a['action_weights']) for a in mock_agents
        ]
...
class TestGenesisAnalysis:
    ...
    def test_analyze_genesis_factors(self, mock_compute, tmp_path):
        ...
        with patch('farm.analysis.genesis.analyze.SessionManager'):
            analyze_genesis_factors(str(tmp_path), ctx, session=mock_session)
        
        # Check output file
        output_file = tmp_path / "genesis_factors.json"
        assert output_file.exists()

    @patch('farm.analysis.genesis.analyze.Path.glob')
    def test_analyze_genesis_across_simulations(self, mock_glob, tmp_path):
        ...
        with patch('farm.analysis.genesis.analyze.compute_initial_state_metrics'):
            analyze_genesis_across_simulations(str(tmp_path.parent), ctx)
        
        # Check output
        output_file = tmp_path / "genesis_comparison.json"
        assert output_file.exists()
    ...
    def test_analyze_critical_period(self, mock_compute, tmp_path):
        ...
        with patch('farm.analysis.genesis.analyze.SessionManager'):
            analyze_critical_period(str(tmp_path), ctx, session=mock_session)
        
        output_file = tmp_path / "critical_period.json"
        assert output_file.exists()
...
class TestGenesisVisualization:
    ...
    @patch('farm.analysis.genesis.plot.plt')
    def test_plot_initial_state_comparison(self, mock_plt, tmp_path):
        ...
        ctx = AnalysisContext(output_path=tmp_path)
        result = plot_initial_state_comparison(metrics, ctx)
        
        assert result is None
        assert mock_plt.savefig.called

    @patch('farm.analysis.genesis.plot.plt')
    def test_plot_critical_period_analysis(self, mock_plt, tmp_path):
        ...
        ctx = AnalysisContext(output_path=tmp_path)
        result = plot_critical_period_analysis(metrics, ctx)
        
        assert result is None
        assert mock_plt.savefig.called

    @patch('farm.analysis.genesis.plot.plt')
    def test_plot_genesis_analysis_results(self, mock_plt, tmp_path):
        ...
        ctx = AnalysisContext(output_path=tmp_path)
        result = plot_genesis_analysis_results(results, ctx)
