...
class TestAdvantageComputations:
    ...
    @patch('farm.analysis.advantage.compute.func')
    @patch('farm.analysis.advantage.compute.SimulationStepModel')
    def test_compute_advantages(self, mock_model, mock_func, mock_session):
        """Test advantage computation."""
        # Mock max step query
        mock_session.query.return_value.scalar.return_value = 300
        
        # Mock agent state count query
        mock_session.query.return_value.scalar.side_effect = [300, 100, 30, 40, 30]
    ...
    @patch('farm.analysis.advantage.compute.func')
    def test_compute_advantages_with_focus_type(self, mock_func, mock_session):
    ...
    @patch('farm.analysis.advantage.compute.func')
    def test_compute_advantages_no_data(self, mock_func, mock_session):
...
class TestEdgeCases:
    ...
    def test_plot_with_data_cleaned_flag(self, sample_advantage_data, sample_advantage_analysis, tmp_path):
        ...
        with patch('farm.analysis.advantage.plot.plt'):
            plot_advantage_results(sample_advantage_data, sample_advantage_analysis, 
                                 str(tmp_path), ctx=ctx, data_cleaned=True)
...
class TestAdvantageHelperFunctions:
    ...
    def test_process_single_simulation(self, mock_session):
        ...
        with patch('farm.analysis.advantage.analyze.compute_comprehensive_dominance') as mock_dom, \
             patch('farm.analysis.advantage.analyze.compute_advantages') as mock_adv, \
             patch('farm.analysis.advantage.analyze.compute_advantage_dominance_correlation') as mock_corr:
            ...
            mock_adv.return_value = {
                'resource_acquisition': {
                    'system_vs_independent': {'advantage': 0.5}
                },
                'composite_advantage': {
                    'system_vs_independent': {
                        'score': 0.6,
                        'components': {'resource': 0.3, 'survival': 0.3}
                    }
                }
            }
