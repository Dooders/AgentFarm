...
class TestDominanceComputations:
    ...
    @patch('farm.analysis.dominance.compute.AgentStateModel')
    @patch('farm.analysis.dominance.compute.func')
    def test_compute_comprehensive_dominance(self, mock_func, mock_model, mock_session, mock_simulation_steps):
...
class TestDominanceAnalysis:
    ...
    def test_process_single_simulation(self, mock_session):
        ...
        with patch('farm.analysis.dominance.analyze.compute_population_dominance') as mock_pop, \
             patch('farm.analysis.dominance.analyze.compute_survival_dominance') as mock_surv, \
             patch('farm.analysis.dominance.analyze.compute_comprehensive_dominance') as mock_comp, \
             patch('farm.analysis.dominance.analyze.compute_dominance_switches') as mock_switch, \
             patch('farm.analysis.dominance.analyze.get_initial_positions_and_resources') as mock_init, \
             patch('farm.analysis.dominance.analyze.get_final_population_counts') as mock_final, \
             patch('farm.analysis.dominance.analyze.get_agent_survival_stats') as mock_stats, \
             patch('farm.analysis.dominance.analyze.get_reproduction_stats') as mock_repro:
            ...
            mock_stats.return_value = {'system_count': 20, 'system_alive': 15, 'system_dead': 5,
                                       'system_avg_survival': 50, 'system_dead_ratio': 0.25}
    ...
    def test_process_dominance_data(self, tmp_path):
        ...
        with patch('farm.analysis.dominance.analyze.setup_and_process_simulations') as mock_setup, \
             patch('farm.analysis.dominance.analyze.analyze_dominance_switch_factors') as mock_factors:
...
class TestDominanceML:
    ...
    def test_run_dominance_classification_insufficient_data(self):
...
class TestEdgeCases:
    ...
    def test_plot_distribution_single_measure(self, tmp_path):
        ...
        df = pd.DataFrame({
            'population_dominance': ['system', 'independent', 'control'] * 3
        })
    ...
    def test_comprehensive_dominance_single_agent_type(self, mock_session):
        ...
        with patch('farm.analysis.dominance.compute.AgentStateModel'), \
             patch('farm.analysis.dominance.compute.func'):
            mock_session.query.return_value.order_by.return_value.all.return_value = steps
...
class TestDominanceHelperFunctions:
    ...
    def test_save_dominance_data_to_db(self, sample_dominance_data):
        ...
        with patch('farm.analysis.dominance.db_io.init_db') as mock_init, \
