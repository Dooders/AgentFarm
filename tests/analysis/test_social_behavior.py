...
class TestSocialBehaviorComputations:
    ...
    def test_compute_resource_sharing_metrics(self, mock_session):
        ...
        assert 'total_shares' in metrics
        assert 'avg_shared_amount' in metrics
    ...
    def test_compute_cooperation_competition_metrics(self, mock_session):
        ...
        mock_coop = [(i, 'share', 10) for i in range(5)]
        # Mock competitive actions (attack)
        mock_comp = [(i, 'attack', 5) for i in range(3)]
        
        def query_side_effect(*args):
            query = MagicMock()
            if hasattr(query, '_cooperation'):
                query.filter.return_value.all.return_value = mock_coop
            else:
                query.filter.return_value.all.return_value = mock_comp
                query._cooperation = True
            return query
    ...
    def test_compute_all_social_metrics(self, mock_session):
        ...
        with patch('farm.analysis.social_behavior.compute.compute_social_network_metrics') as mock_network, \
             patch('farm.analysis.social_behavior.compute.compute_resource_sharing_metrics') as mock_sharing, \
             patch('farm.analysis.social_behavior.compute.compute_spatial_clustering') as mock_spatial, \
             patch('farm.analysis.social_behavior.compute.compute_cooperation_competition_metrics') as mock_coop:
            
            mock_network.return_value = {'total_interactions': 100}
            mock_sharing.return_value = {'total_shares': 50}
            mock_spatial.return_value = {'clustering_score': 0.7}
            mock_coop.return_value = {'cooperation_ratio': 0.6}
...
class TestSocialBehaviorAnalysis:
    ...
    @patch('farm.analysis.social_behavior.analyze.SessionManager')
    @patch('farm.analysis.social_behavior.analyze.compute_all_social_metrics')
    def test_analyze_social_behaviors(self, mock_compute, mock_session_mgr, tmp_path):
    ...
    @patch('farm.analysis.social_behavior.analyze.Path.glob')
    @patch('farm.analysis.social_behavior.analyze.SessionManager')
    @patch('farm.analysis.social_behavior.analyze.compute_all_social_metrics')
    def test_analyze_social_behaviors_across_simulations(self, mock_compute, mock_session, mock_glob, tmp_path):
        ...
        output_file = tmp_path / "social_behaviors_comparison.json"
        assert output_file.exists()
...
class TestDataProcessing:
    ...
    @patch('farm.analysis.social_behavior.data.SessionManager')
    def test_load_social_behavior_data_from_db(self, mock_session_mgr, tmp_path):
        """Test loading social behavior data from database."""
        db_path = tmp_path / "simulation.db"
        db_path.touch()
        
        mock_session = MagicMock()
        mock_session_mgr.return_value.session.return_value.__enter__.return_value = mock_session
        
        # Mock query results
        mock_session.query.return_value.filter.return_value.all.return_value = []
        
        data = load_social_behavior_data_from_db(tmp_path)
...
class TestEdgeCases:
    ...
    def test_analyze_with_progress_callback(self, tmp_path):
        ...
        with patch('farm.analysis.social_behavior.analyze.SessionManager'), \
             patch('farm.analysis.social_behavior.analyze.compute_all_social_metrics'):
            analyze_social_behaviors(str(tmp_path), ctx)
    ...
    def test_insights_extraction_minimal_data(self):
        ...
        minimal_metrics = {
            'social_network': {'total_interactions': 1}
        }
