...
from farm.actions.algorithms.mlp import MLPActionSelector
from farm.actions.algorithms.ensemble import RandomForestActionSelector


class TestMLPActionSelector(unittest.TestCase):
    def test_unfitted_returns_uniform(self):
        algo = MLPActionSelector(num_actions=4, hidden_layer_sizes=(8,), random_state=42, max_iter=100)
        state = np.zeros(6)
    ...
    def test_train_and_select(self):
        ...
        algo = MLPActionSelector(num_actions=4, hidden_layer_sizes=(8,), random_state=42, max_iter=200)
        algo.train(X, y)
...
class TestRandomForestActionSelector(unittest.TestCase):
    def test_unfitted_returns_uniform(self):
        algo = RandomForestActionSelector(num_actions=3, n_estimators=10, random_state=0)
        state = np.ones(5)
        proba = algo.predict_proba(state)
