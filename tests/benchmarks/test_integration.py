...
from benchmarks.core.spec import RunSpec, load_spec
...
class TestBenchmarkIntegration(unittest.TestCase):
    ...
    @patch("benchmarks.core.runner.time_block")
    def test_runner_with_multiple_instruments(self, mock_time_block):
        ...
        with patch(
            "benchmarks.core.runner.cprofile_capture"
        ) as mock_cprofile, patch(
            "benchmarks.core.runner.psutil_sampling"
        ) as mock_psutil:
        ...
        self.assertGreaterEqual(
            mock_time_block.call_count, 1
        )  # At least 1 measured iteration
    ...
    def test_registry_experiment_integration(self):
        ...
        self.assertEqual(experiment.data_size, 25)
    ...
    def test_full_benchmark_workflow(self, mock_time_block):
        ...
        self.assertIn("Full workflow test", result.notes)
    ...
    def test_artifact_creation_integration(self, mock_time_block):
        ...
        with patch(
            "benchmarks.core.instrumentation.cprofile.cprofile_capture"
        ) as mock_cprofile:
            mock_cprofile.return_value.__enter__ = Mock()
            mock_cprofile.return_value.__exit__ = Mock(return_value=None)
            ...
            def mock_cprofile_side_effect(run_dir, name, iteration, metrics, **kwargs):
                metrics["cprofile_artifact"] = os.path.join(
                    run_dir, f"{name}_iter{iteration:03d}.prof"
                )
                metrics["cprofile_summary_path"] = os.path.join(
                    run_dir, f"{name}_iter{iteration:03d}_summary.json"
                )
                return Mock()
