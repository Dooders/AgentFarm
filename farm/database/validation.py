...
class IntegrityValidator:
    ...
    def validate_table_existence(self) -> None:
        ...
        required_tables = {
            "agents", "agent_states", "simulation_steps", "agent_actions",
            "resource_states", "reproduction_events", "health_incidents",
            "learning_experiences", "social_interactions", "interactions",
    ...
    def validate_primary_keys(self) -> None:
        ...
        # Check agents table
        duplicate_agents = self.session.execute(text("""
        ...
        duplicate_states = self.session.execute(text("""
    ...
    def validate_foreign_keys(self) -> None:
        ...
        orphaned_states = self.session.execute(text("""
        ...
        orphaned_actions = self.session.execute(text("""
    ...
    def validate_not_null_constraints(self) -> None:
        ...
        # Check agents table
        null_agent_ids = self.session.execute(text("""
        ...
        null_state_ids = self.session.execute(text("""
    ...
    def validate_temporal_consistency(self) -> None:
        ...
        invalid_lifespans = self.session.execute(text("""
        ...
        invalid_steps = self.session.execute(text("""
...
class StatisticalValidator:
    ...
    def validate_health_ranges(self) -> None:
        ...
        invalid_health = self.session.execute(text("""
            SELECT COUNT(*) FROM agent_states 
    ...
    def validate_resource_ranges(self) -> None:
        ...
        negative_resources = self.session.execute(text("""
            SELECT COUNT(*) FROM agent_states 
        ...
        negative_resource_amounts = self.session.execute(text("""
    ...
    def validate_generation_monotonicity(self) -> None:
        ...
        invalid_generations = self.session.execute(text("""
    ...
    def validate_population_consistency(self) -> None:
        ...
        # Get the latest step
        latest_step = self.session.execute(text("""
    ...
    def validate_step_continuity(self) -> None:
        ...
        # Get step numbers
        steps = self.session.execute(text("""
