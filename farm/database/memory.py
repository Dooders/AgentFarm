...
class AgentMemory:
    ...
    def remember_state(
        ...
        try:
            ...
            memory_entry = {
                "timestamp": time.time(),
                "step": step,
                "state": state.as_dict() if hasattr(state, "as_dict") else vars(state),
                "priority": priority,
            }
            ...
            if perception is not None:
                memory_entry["perception"] = perception.as_serializable() if hasattr(perception, "as_serializable") else vars(perception)
            if metadata:
    ...
    def retrieve_recent_states(self, count: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to retrieve recent states for agent {self.agent_id}: {e}")
            return []
    ...
    def search_by_state_value(
        ...
        try:
            ...
            logger.error(f"Failed to search state values for agent {self.agent_id}: {e}")
            return []
    ...
    def search_by_position(
        ...
        try:
            ...
            for step in all_steps:
                state_data = self.retrieve_state(int(step))
                if (not state_data or "state" not in state_data or 
                        "position_x" not in state_data["state"] or 
                        "position_y" not in state_data["state"]):
                    continue
    ...
    def _cleanup_old_memories(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Cleaned up {len(oldest_steps)} old memories for agent {self.agent_id}")
...
def integrate_with_agent():
    ...
    memory_config = RedisMemoryConfig(
        ...
        memory_limit=self.config.memory_limit if hasattr(self.config, 'memory_limit') else 1000
    )
    ...
    """
    def recall_similar_experiences(self, position=None, action=None, limit=5):
        '''Retrieve memories similar to current situation.'''
        if not hasattr(self, 'memory'):
            return []
            
        # Search by position if provided
        if position:
            return self.memory.search_by_position(position, radius=10.0, limit=limit)
            
        # Search by action if provided
        if action:
            results = []
            for state in self.memory.retrieve_recent_states(100):
