...
def format_diff_output(diff: SimulationDifference) -> Dict[str, Any]:
    ...
    """
    report = {"metadata": {}, "parameters": {}, "results": {}, "metrics": {}}

    # Format metadata differences
    for field, (val1, val2) in diff.metadata_diff.items():
        report["metadata"][field] = {"simulation_1": val1, "simulation_2": val2}
...
def compare_simulations(session, sim1_id: int, sim2_id: int) -> Dict[str, Any]:
    ...
    # Always return formatted output
...
def summarize_comparison(session, sim1_id: int, sim2_id: int) -> str:
    ...
    if differences["metadata"]:
        ...
        for field, values in differences["metadata"].items():
            summary.append(
                f"  {field}: {values['simulation_1']} -> {values['simulation_2']}"
            )
