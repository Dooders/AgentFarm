...
def validate_chart(chart_name: str, show_plot: bool = True):
    ...
    """
    # Initialize database and analyzer without saving files
    database = SimulationDatabase("simulations/simulation.db")
    analyzer = ChartAnalyzer(database, save_charts=False)
    ...
    chart_functions = {
        ...
        "births_and_deaths_by_type": lambda: plot_births_and_deaths_by_type(
        ...
        "resource_distribution_entropy": lambda: plot_resource_distribution_entropy(
            simulation_df
        ),
        "rewards": lambda: plot_rewards(simulation_df),
        "average_resources": lambda: plot_average_resources(simulation_df),
        "agent_lifespan_histogram": lambda: plot_agent_lifespan_histogram(
            simulation_df, "sqlite:///simulations/simulation.db"
        ),
        "agent_type_comparison": lambda: plot_agent_type_comparison(
            simulation_df, "sqlite:///simulations/simulation.db"
        ),
        # Action charts
        ...
        "action_frequency_over_time": lambda: plot_action_frequency_over_time(
            actions_df
        ),
        "rewards_over_time": lambda: plot_rewards_over_time(actions_df),
        "action_target_distribution": lambda: plot_action_target_distribution(
            actions_df
        ),
        # Agent charts
