...
class MoveModule(BaseDQNModule):
    ...
    def __init__(self, config: MoveConfig = DEFAULT_MOVE_CONFIG, device: torch.device = DEVICE, db: Optional["SimulationDatabase"] = None, shared_encoder: Optional[SharedEncoder] = None) -> None:
        super().__init__(input_dim=6, output_dim=4, config=config, device=device, db=db)
        self.q_network = MoveQNetwork(input_dim=6, hidden_size=config.dqn_hidden_size, shared_encoder=shared_encoder).to(device)
        self.target_network = MoveQNetwork(input_dim=6, hidden_size=config.dqn_hidden_size, shared_encoder=shared_encoder).to(device)
