    ...
    def update(self) -> None:
        ...
        try:
            ...
            if self.time % 100 == 0 and self.time > 0:
                ...
                health_values = [
                    combat.health for a in self._agent_objects.values() if (combat := a.get_component("combat"))
                ]
                ...
                resource_values = [
                    resource.level for a in self._agent_objects.values() if (resource := a.get_component("resource"))
                ]
    ...
    def _process_action(self, agent_id: str, action: Optional[int]) -> None:
        ...
        if action_name:
            ...
            if action_obj:
                ...
                resources_before = (
                    agent.get_component("resource").level if (agent and agent.get_component("resource")) else None
                )
