...
class RewardComponent(AgentComponent):
    ...
    def on_terminate(self) -> None:
        ...
        if self.core and self.core.alive:
            # Apply death penalty
    ...
    def _calculate_state_reward(self) -> float:
        ...
        # Resource reward
        resource_reward = current_state["resource_level"] * self.config.resource_reward_scale
        
        # Health reward
        health_reward = current_state["health"] * self.config.health_reward_scale
        
        # Survival reward
        survival_reward = self.config.survival_bonus if current_state["alive"] else self.config.death_penalty
        
        # Age reward
        age_reward = current_state["age"] * self.config.age_bonus
