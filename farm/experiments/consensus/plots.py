...
def _gap_vs_loser_share(trials: pd.DataFrame, path: Path) -> None:
    fig, panels = _population_axes(trials, "Supporter-loser gap vs. loser share (per trial)")
    for population, ax in panels:
        subset = trials[trials["population"] == population]
...
def _lambda_by_paradigm(trials: pd.DataFrame, path: Path) -> None:
    ...
    for population, ax in panels:
        subset = trials[trials["population"] == population]
        paradigms = list(subset["paradigm"].unique())
        data = [subset.loc[subset["paradigm"] == p, "lambda_winner"].to_numpy() for p in paradigms]
        ...
        ax.set_ylabel("λ of winner (pre-cap)")
        ax.set_title(population)
...
def write_figures(trials: pd.DataFrame, figures_dir: Path) -> None:
    ...
    _welfare_by_paradigm(trials, figures_dir / "welfare_by_paradigm.png")
    _gap_vs_loser_share(trials, figures_dir / "gap_vs_loser_share.png")
    _lambda_by_paradigm(trials, figures_dir / "lambda_by_paradigm.png")
