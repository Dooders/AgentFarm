...
from farm.api.models import (
    SimulationStatus, SimulationResults, ExperimentStatus, ExperimentResults,
...
class UnifiedAdapter:
    ...
    def start_simulation(self, simulation_id: str) -> SimulationStatus:
        ...
        try:
            ...
            def on_step(step_num):
                ...
                self._emit_event("simulation_step", simulation_id=simulation_id, 
                               data={"step": step_num, "total": sim_info["total_steps"]})
            
            def on_status(status):
                sim_info["status"] = SimulationStatus(status)
                self._emit_event("simulation_status_change", simulation_id=simulation_id, 
                               data={"status": status})
            ...
            self._emit_event("simulation_error", simulation_id=simulation_id, 
                           data={"error": str(e)})
            raise
    ...
    def get_simulation_status(self, simulation_id: str) -> SimulationStatus:
        ...
        # Calculate progress
        progress = (sim_info["current_step"] / sim_info["total_steps"]) * 100 if sim_info["total_steps"] > 0 else 0
    ...
    def get_simulation_results(self, simulation_id: str) -> SimulationResults:
        ...
        if sim_dir.exists():
            for file_path in sim_dir.rglob("*"):
                if file_path.is_file() and file_path.suffix in [".db", ".csv", ".json", ".png", ".html"]:
        ...
        return SimulationResults(
            ...
            metrics={
                "duration_seconds": (sim_info["end_time"] - sim_info["start_time"]).total_seconds() 
                if sim_info["start_time"] and sim_info["end_time"] else 0,
                "steps_per_second": sim_info["current_step"] / 
                ((sim_info["end_time"] - sim_info["start_time"]).total_seconds() 
                if sim_info["start_time"] and sim_info["end_time"] else 1)
            },
    ...
    def create_experiment(self, config: Dict[str, Any]) -> str:
        ...
        base_config = self.config_manager.convert_to_simulation_config(config.get("base_config", {}))
        if not base_config:
        ...
        try:
            ...
            def run_experiment():
                try:
                    ...
                    self._emit_event("experiment_completed", experiment_id=experiment_id)
                    
                except Exception as e:
                    exp_info["status"] = ExperimentStatus.ERROR
                    exp_info["error_message"] = str(e)
                    self._emit_event("experiment_error", experiment_id=experiment_id, 
                                   data={"error": str(e)})
    ...
    def get_experiment_status(self, experiment_id: str) -> ExperimentStatus:
        ...
        # Calculate progress
        progress = (state["current_iteration"] / exp_info["total_iterations"]) * 100 if exp_info["total_iterations"] > 0 else 0
    ...
    def get_experiment_results(self, experiment_id: str) -> ExperimentResults:
        ...
        return ExperimentResults(
            ...
            results_summary={
                "duration_seconds": (exp_info["end_time"] - exp_info["start_time"]).total_seconds() 
                if exp_info["start_time"] and exp_info["end_time"] else 0
            },
            data_files=data_files,
            analysis_available=len(data_files) > 0
        )
    ...
    def compare_simulations(self, simulation_ids: List[str]) -> ComparisonResults:
        ...
        compare_simulations(
            ...
            analysis_path=str(comparison_dir)
        )
    ...
    def get_event_history(self, subscription_id: str) -> List[Event]:
        ...
        for event in self._event_history:
            ...
            if subscription.simulation_id and event.simulation_id != subscription.simulation_id:
                continue
            
            # Check experiment filter
            if subscription.experiment_id and event.experiment_id != subscription.experiment_id:
                continue
    ...
    def _emit_event(self, event_type: str, simulation_id: Optional[str] = None,
                   experiment_id: Optional[str] = None, data: Optional[Dict] = None):
        """Emit an event.
