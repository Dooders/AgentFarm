...
def basic_simulation_example():
    ...
    try:
        # Create session
        session_id = controller.create_session("Basic Simulation", "Simple simulation example")
        ...
        while True:
            ...
            print(f"Step {status.current_step}/{status.total_steps} ({status.progress_percentage:.1f}%)")
...
def parameter_variation_experiment_example():
    ...
    try:
        # Create session
        session_id = controller.create_session("Parameter Study", "Testing different agent populations")
        print(f"Created session: {session_id}")
        ...
        config = {
            ...
            "variations": [
                {"agents": {"system_agents": 5, "independent_agents": 15}},  # 25% system
                {"agents": {"system_agents": 8, "independent_agents": 12}},  # 40% system
                {"agents": {"system_agents": 10, "independent_agents": 10}}, # 50% system
                {"agents": {"system_agents": 12, "independent_agents": 8}},  # 60% system
                {"agents": {"system_agents": 15, "independent_agents": 5}}   # 75% system
            ]
        }
        ...
        while True:
            ...
            print(f"Iteration {status.current_iteration}/{status.total_iterations} ({status.progress_percentage:.1f}%)")
        ...
        print(f"Completed iterations: {results.completed_iterations}/{results.total_iterations}")
...
def configuration_templates_example():
    ...
    try:
        ...
        config = controller.create_config_from_template("combat_simulation", {
            "steps": 1000,
...
def analysis_and_comparison_example():
    ...
    try:
        # Create session
        session_id = controller.create_session("Analysis Demo", "Testing analysis features")
        print(f"Created session: {session_id}")
