...
def analyze_genesis_factors(session) -> Dict[str, Any]:
    ...
    try:
        ...
        results["database_schema"] = {
            "has_action_weights_column": has_action_weights
        }
        ...
        if has_action_weights:
            ...
            logger.warning("Database schema does not have action_weights column. Some analysis features may be limited.")
        ...
        try:
            ...
            logger.info(f"Initial metrics computed and transformed: {list(initial_metrics.keys())}")
...
def extract_genesis_insights(
    ...
    if "overall_impact_scores" in impact_scores:
        ...
        sorted_factors = sorted(impact_factors.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    if "critical_period_dominant" in critical_period_metrics:
        ...
        if initial_advantages and "dominant_agent_type" in initial_advantages:
            ...
            insights["initial_advantage_realized"] = (critical_dominant == initial_dominant)
            insights["determinism_level"] = "high" if insights["initial_advantage_realized"] else "low"
    ...
    if "initial_relative_advantages" in initial_metrics:
        ...
        if "resource_proximity_advantage" in relative_advantages:
            ...
            if net_advantages:
                ...
                advantages["dominant_agent_type"] = max(net_advantages, key=net_advantages.get)
    ...
    if "agent_starting_attributes" in initial_metrics:
        ...
        for agent_type, attributes in starting_attributes.items():
            if "initial_resources" in attributes:
                initial_resources[agent_type] = attributes["initial_resources"].get("mean", 0)
        
        if initial_resources:
            advantages["initial_resources"] = initial_resources
            advantages["resource_advantage_type"] = max(initial_resources, key=initial_resources.get)
    
    return advantages


def identify_critical_events(critical_period_metrics: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
...
def analyze_cross_simulation_patterns(simulation_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    ...
    for sim in simulation_data:
        ...
        if "dominant_agent_type" in advantages and "resource_proximity_net_advantages" in advantages:
    ...
    if advantage_data:
        ...
        if consistency:
            patterns["most_consistent_advantage"] = max(consistency.items(), key=lambda x: x[1])
        ...
        for data in critical_events:
            for event in data["events"]:
                if event["event_type"] == "first_reproduction":
                    ...
                    first_reproduction[agent_type] = first_reproduction.get(agent_type, 0) + 1
...
def analyze_determinism(simulation_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if total_count > 0:
        determinism["initial_advantage_realization_rate"] = advantage_realized_count / total_count
...
def analyze_critical_period(session, critical_period_end: int = 100) -> Dict[str, Any]:
    ...
    predictive_power = analyze_critical_period_predictive_power(session, metrics, critical_period_end)
...
def analyze_critical_period_predictive_power(
    ...
    if any(agent_counts.values()):
        final_dominant = max(agent_counts, key=agent_counts.get)
