...
def transform_metrics_for_plotting(metrics: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "agent_type_resource_proximity" in metrics:
        transformed["agent_type_resource_proximity"] = metrics["agent_type_resource_proximity"]
...
def compute_initial_state_metrics(session: Session) -> Dict[str, Any]:
    ...
    initial_agents = session.query(AgentModel).filter(AgentModel.birth_time == 0).all()
    
    # Get initial resources (step_number = 0)
    initial_resources = session.query(ResourceModel).filter(ResourceModel.step_number == 0).all()
    
    # Store agent types for reference
    metrics["agent_types"] = {agent.agent_id: agent.agent_type for agent in initial_agents}
    ...
    for agent_type in agent_type_counts.keys():
        agent_type_stats[agent_type] = {
            ...
            "avg_initial_resources": np.mean(agent_type_resources[agent_type]) if agent_type_resources[agent_type] else 0.0,
            "avg_starting_health": np.mean(agent_type_health[agent_type]) if agent_type_health[agent_type] else 0.0
        }
    
    metrics["agent_starting_attributes"] = agent_type_stats
    
    # Total initial resources in environment
    total_resource_amount = sum(resource.amount for resource in initial_resources)
    metrics["total_resource_amount"] = total_resource_amount
    
    # Average resource amount
    metrics["avg_resource_amount"] = total_resource_amount / len(initial_resources) if initial_resources else 0
    
    # Resource distribution metrics
    if initial_resources:
        resource_amounts = [resource.amount for resource in initial_resources]
        ...
        resource_positions = [(resource.position_x, resource.position_y) for resource in initial_resources]
        ...
        if len(resource_positions) >= 2:
            ...
            connections = np.sum(adjacency_matrix, axis=1) - 1  # Subtract 1 to exclude self-connection
    ...
    agent_resource_metrics = compute_agent_resource_proximity(initial_agents, initial_resources)
    ...
    advantage_metrics = compute_initial_relative_advantages(initial_agents, initial_resources)
    metrics.update(advantage_metrics)
    
    return metrics


def compute_agent_resource_proximity(agents: List[AgentModel], resources: List[ResourceModel]) -> Dict[str, Any]:
    """
    ...
    for agent in agents:
        ...
        agent_resource_distances[agent.agent_id] = {
            ...
            "weighted_avg_distance": np.mean(weighted_distances) if weighted_distances else float('inf'),
            "resources_within_30": sum(1 for d in distances if d <= 30),
            "resource_amount_within_30": sum(r.amount for r, d in zip(resources, distances) if d <= 30)
        }
...
def compute_agent_agent_proximity(agents: List[AgentModel]) -> Dict[str, Any]:
    """
    ...
    for i, agent in enumerate(agents):
        ...
        agent_agent_proximity[agent.agent_id] = {
            "nearest_agent_distance": min(other_distances) if other_distances else float('inf'),
            "avg_agent_distance": np.mean(other_distances) if other_distances else float('inf'),
...
def compute_agent_starting_attributes(agents: List[AgentModel]) -> Dict[str, Any]:
    """
    ...
    has_action_weights = any(hasattr(agent, 'action_weights') and agent.action_weights is not None 
                              for agent in agents)
    ...
    for agent in agents:
        agent_type_attributes[agent.agent_type]["initial_resources"].append(agent.initial_resources)
        agent_type_attributes[agent.agent_type]["starting_health"].append(agent.starting_health)
        agent_type_attributes[agent.agent_type]["starvation_threshold"].append(agent.starvation_threshold)
        
        # Process action_weights if they exist
        if has_action_weights and hasattr(agent, 'action_weights') and agent.action_weights is not None:
            ...
            agent_type_attributes[agent.agent_type]["action_weights_data"].append(agent.action_weights)
            ...
            try:
                ...
                if isinstance(agent.action_weights, dict):
                    ...
                    agent_type_attributes[agent.agent_type]["action_count"].append(len(agent.action_weights))
                    
                    # Average weight value
                    if "avg_action_weight" not in agent_type_attributes[agent.agent_type]:
                        agent_type_attributes[agent.agent_type]["avg_action_weight"] = []
                    if agent.action_weights:
                        avg_weight = sum(float(w) for w in agent.action_weights.values()) / len(agent.action_weights)
                        agent_type_attributes[agent.agent_type]["avg_action_weight"].append(avg_weight)
                    
                    # Maximum weight
                    if "max_action_weight" not in agent_type_attributes[agent.agent_type]:
                        agent_type_attributes[agent.agent_type]["max_action_weight"] = []
                    if agent.action_weights:
                        max_weight = max(float(w) for w in agent.action_weights.values())
                        agent_type_attributes[agent.agent_type]["max_action_weight"].append(max_weight)
            except Exception as e:
                # Log but continue if there's an error processing action_weights
                logger.warning(f"Error processing action_weights for agent {agent.agent_id}: {e}")
    ...
    if has_action_weights:
        for agent_type, attributes in agent_type_attributes.items():
            if "action_weights_data" in attributes and attributes["action_weights_data"]:
                # Initialize action weight analysis
                if "action_weights_analysis" not in agent_starting_attributes[agent_type]:
                    agent_starting_attributes[agent_type]["action_weights_analysis"] = {}
                ...
                for action in action_types:
                    ...
                    if action_values:
                        agent_starting_attributes[agent_type]["action_weights_analysis"][action] = {
                            "mean": np.mean(action_values),
                            "median": np.median(action_values),
                            "min": min(action_values),
                            "max": max(action_values),
                            "std": np.std(action_values) if len(action_values) > 1 else 0.0
                        }
...
def compute_initial_relative_advantages(agents: List[AgentModel], resources: List[ResourceModel]) -> Dict[str, Any]:
    """
    ...
    for agent_type, agent_list in agents_by_type.items():
        ...
        if agent_list:
            resources_in_range_by_type[agent_type] = resources_in_range / len(agent_list)
            resource_amount_in_range_by_type[agent_type] = resource_amount_in_range / len(agent_list)
    ...
    for i, type1 in enumerate(agent_types):
        for j in range(i+1, len(agent_types)):
            ...
            if type1 not in resources_in_range_by_type or type2 not in resources_in_range_by_type:
                continue
            
            # Calculate resource count advantage
            count_advantage = resources_in_range_by_type[type1] - resources_in_range_by_type[type2]
            
            # Calculate resource amount advantage
            amount_advantage = resource_amount_in_range_by_type[type1] - resource_amount_in_range_by_type[type2]
    ...
    for i, type1 in enumerate(agent_types):
        for j in range(i+1, len(agent_types)):
            ...
            avg_resources_type1 = np.mean([agent.initial_resources for agent in agents_by_type[type1]])
            avg_resources_type2 = np.mean([agent.initial_resources for agent in agents_by_type[type2]])
            
            # Calculate average starting health for each type
            avg_health_type1 = np.mean([agent.starting_health for agent in agents_by_type[type1]])
            avg_health_type2 = np.mean([agent.starting_health for agent in agents_by_type[type2]])
            
            # Store advantages
            key = f"{type1}_vs_{type2}"
            attribute_advantage[key] = {
                "initial_resources_advantage": avg_resources_type1 - avg_resources_type2,
...
def compute_simulation_outcomes(session: Session) -> Dict[str, Any]:
    ...
    final_step = session.query(SimulationStepModel).order_by(SimulationStepModel.step_number.desc()).first()
    ...
    if any(agent_counts.values()):
        outcomes["population_dominant_type"] = max(agent_counts, key=agent_counts.get)
    ...
    if reproduction_events:
        ...
        successful_reproductions = [event for event in reproduction_events if event.success]
        ...
        for event in successful_reproductions:
            parent = session.query(AgentModel).filter(AgentModel.agent_id == event.parent_id).first()
            if parent:
    ...
    if agents:
        ...
        if dead_agents:
            lifespans = [agent.death_time - agent.birth_time for agent in dead_agents]
        ...
        for agent_type, counts in survival_by_type.items():
            if counts["total"] > 0:
                outcomes[f"{agent_type}_survival_rate"] = counts["alive"] / counts["total"]
...
def extract_features_from_metrics(metrics: Dict[str, Any]) -> Dict[str, float]:
    ...
    def flatten_dict(d, prefix=""):
        for key, value in d.items():
            ...
            elif isinstance(value, (int, float)) and not isinstance(value, bool) and not math.isnan(value):
...
def calculate_feature_importance(features: Dict[str, float], outcome: float) -> Dict[str, float]:
    """
...
def compute_critical_period_metrics(session: Session, critical_period_end: int = 100) -> Dict[str, Any]:
    """
    ...
    # Get initial state
    initial_metrics = compute_initial_state_metrics(session)
    initial_agent_count = sum(initial_metrics.get("agent_type_distribution", {}).values())
    ...
    population_metrics = {
        "system_agents": [step.system_agents for step in critical_steps],
        "independent_agents": [step.independent_agents for step in critical_steps],
        "control_agents": [step.control_agents for step in critical_steps],
        "total_agents": [step.total_agents for step in critical_steps]
    }
    ...
    for agent_type in ["system_agents", "independent_agents", "control_agents"]:
        if len(population_metrics[agent_type]) >= 2:
            initial_pop = population_metrics[agent_type][0]
            final_pop = population_metrics[agent_type][-1]
    ...
    metrics["survival_rate"] = 1.0 - (total_early_deaths / initial_agent_count) if initial_agent_count > 0 else 0.0
    ...
    for event in reproduction_events:
        agent = session.query(AgentModel).filter(AgentModel.agent_id == event.parent_id).first()
        if agent:
    ...
    metrics["reproduction_rate"] = reproduction_count / initial_agent_count if initial_agent_count > 0 else 0.0
    ...
    for state in agent_states:
        agent = session.query(AgentModel).filter(AgentModel.agent_id == state.agent_id).first()
    ...
    if social_interactions:
        ...
        for interaction in social_interactions:
            initiator = session.query(AgentModel).filter(AgentModel.agent_id == interaction.initiator_id).first()
            recipient = session.query(AgentModel).filter(AgentModel.agent_id == interaction.recipient_id).first()
            
            if initiator and recipient:
                interaction_counts[initiator.agent_type][interaction.interaction_type] += 1
    ...
    if critical_steps:
        ...
        agent_counts = {
            "SystemAgent": last_critical_step.system_agents,
            "IndependentAgent": last_critical_step.independent_agents,
            "ControlAgent": last_critical_step.control_agents
        }
        
        if any(agent_counts.values()):
            metrics["critical_period_dominant"] = max(agent_counts, key=agent_counts.get)
