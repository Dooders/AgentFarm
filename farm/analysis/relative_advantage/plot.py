"""
Relative Advantage Plotting Module

This module provides functions to visualize relative advantage data and analysis results,
...
def plot_relative_advantage_results(df: pd.DataFrame, analysis_results: Dict[str, Any], output_path: str, data_cleaned: bool = False):
    """
    Generate visualizations for relative advantage analysis results.
...
def plot_advantage_correlation_matrix(df: pd.DataFrame, output_path: str, data_cleaned: bool = False):
    """
    ...
    plt.title("Correlation Between Relative Advantages and Dominance Scores", fontsize=14)
    plt.tight_layout()
...
def plot_advantage_category_importance(analysis_results: Dict[str, Any], output_path: str, data_cleaned: bool = False):
    """
    ...
    for category, agent_data in category_data.items():
        ...
        system_scores.append(
            agent_data.get("system", {}).get("average_relevance", 0)
        )
        independent_scores.append(
            agent_data.get("independent", {}).get("average_relevance", 0)
        )
        control_scores.append(
            agent_data.get("control", {}).get("average_relevance", 0)
        )
...
def plot_top_dominance_predictors(analysis_results: Dict[str, Any], output_path: str, data_cleaned: bool = False):
    """
    ...
    for agent_type, data in agent_type_data.items():
        ...
        for adv_col, pred_data in top_predictors:
            ...
            for cat in ["resource acquisition", "reproduction", "survival", 
                       "population growth", "combat", "initial positioning"]:
                if cat in label:
        ...
        # Save the figure
        output_file = os.path.join(output_path, f"{agent_type}_dominance_predictors.png")
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        plt.close()
        logging.info(f"Saved {agent_type} dominance predictors to {output_file}")


def plot_advantage_thresholds(analysis_results: Dict[str, Any], output_path: str, data_cleaned: bool = False):
    """
    ...
    for agent_type, thresholds in threshold_data.items():
        ...
        fig, axes = plt.subplots(len(top_thresholds), 1, figsize=(10, 4 * len(top_thresholds)))
        ...
        plt.suptitle(f'Critical Advantage Thresholds for {agent_type.capitalize()} Dominance', fontsize=16)
        ...
        # Save the figure
        output_file = os.path.join(output_path, f"{agent_type}_advantage_thresholds.png")
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        plt.close()
        logging.info(f"Saved {agent_type} advantage thresholds to {output_file}")


def plot_advantage_timing(analysis_results: Dict[str, Any], output_path: str, data_cleaned: bool = False):
    """
    ...
    phase_strengths = {
        agent_type: {
            ...
            for phase, advantages in data.items() if advantages
        }
    ...
    for i, agent_type in enumerate(agent_types):
        ...
        plt.bar(x + offsets[i], values, width, label=f'{agent_type.capitalize()}', color=colors[i % len(colors)])
    
    plt.xlabel('Simulation Phase')
    plt.ylabel('Average Advantage Strength')
    plt.title('Relative Advantage Strength by Simulation Phase')
...
def plot_composite_advantage_breakdown(df: pd.DataFrame, output_path: str, data_cleaned: bool = False):
    """
    ...
    for pair, components in pair_components.items():
        ...
        plt.yticks(range(len(component_names)), [c.replace('_', ' ').title() for c in component_names])
...
def plot_advantage_trajectories(df: pd.DataFrame, output_path: str, data_cleaned: bool = False):
    """
    ...
    for col in trajectory_cols:
        for category in [
            "resource_acquisition", "reproduction", "survival", 
            "population_growth", "combat"
        ]:
            if category in col:
    ...
    for category, cols in category_trajectories.items():
        ...
        for pair in pairs:
            ...
            plt.errorbar(
                x_pos, pair_trajectories, yerr=pair_std,
                fmt='o', capsize=5, 
