"""
Relative Advantage Analysis Module

This module provides functions to analyze relative advantages across multiple simulations,
...
from farm.analysis.relative_advantage.compute import (
    compute_relative_advantages,
    compute_advantage_dominance_correlation
)


def analyze_relative_advantages(experiment_path: str) -> pd.DataFrame:
    """
    Analyze relative advantages across all simulations in an experiment.
    ...
    -------
    pandas.DataFrame
        DataFrame containing relative advantage metrics for all simulations
    """
    data = []
    ...
    for folder in sim_folders:
        ...
        # Log progress every 10 simulations or for the first few
        if processed_count <= 5 or processed_count % 10 == 0 or processed_count == total_simulations:
            elapsed_time = time.time() - start_time
            avg_time_per_sim = elapsed_time / processed_count if processed_count > 0 else 0
            estimated_remaining = avg_time_per_sim * (total_simulations - processed_count)
        ...
        try:
            ...
            # Compute comprehensive dominance
            ...
            if not dominance_result:
                logging.warning(f"Skipping {folder}: Failed to compute dominance")
                session.close()
                error_count += 1
                continue
            ...
            logging.debug(f"Computing relative advantages for iteration {iteration}")
            advantages = compute_relative_advantages(session)
            ...
            for agent_type in ["system", "independent", "control"]:
                sim_data[f"{agent_type}_dominance_score"] = dominance_result["scores"][agent_type]
            ...
            # Add relative advantage to dominance correlation
            logging.debug(f"Computing advantage-dominance correlation for iteration {iteration}")
            advantage_correlation = compute_advantage_dominance_correlation(session)
            if advantage_correlation:
                sim_data["advantage_ratio"] = advantage_correlation["summary"]["advantage_ratio"]
                
                # Add summary statistics
                sim_data["advantages_favoring_dominant"] = advantage_correlation["summary"]["advantages_favoring_dominant"]
                sim_data["total_advantages"] = advantage_correlation["summary"]["total_advantages"]
...
def analyze_advantage_patterns(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Analyze patterns in relative advantages across simulations.
    ...
    advantage_cols = [col for col in df.columns if (
        ("advantage" in col or "trajectory" in col) and 
        "composite" not in col and 
        "ratio" not in col
    )]
    ...
    # 3. Analyze relative importance of different advantage categories
    categories = [
    ...
    for agent_type in ["system", "independent", "control"]:
        ...
        for phase in ["early", "mid", "late"]:
            ...
            phase_cols = [
                col for col in advantage_cols 
                if f"{phase}_phase_advantage" in col or 
                   (phase == "early" and "first_" in col)  # Special case for first reproduction
            ]
            ...
            for col in phase_cols:
                ...
                try:
                    ...
                    relative_strength = avg_value / all_avg if all_avg != 0 else float('inf')
    ...
    for agent_type in ["system", "independent", "control"]:
        ...
        for col in advantage_cols:
            if f"{agent_type}_vs_" in col or f"_vs_{agent_type}" in col:
                try:
                    ...
                    if len(agent_dominant[col].dropna()) > 5 and len(df[df["dominant_type"] != agent_type][col].dropna()) > 5:
                    ...
                    try:
                        pooled_std = np.sqrt((
                            (len(agent_dominant[col].dropna()) - 1) * agent_dominant[col].std()**2 +
                            (len(df[df["dominant_type"] != agent_type][col].dropna()) - 1) * 
                            df[df["dominant_type"] != agent_type][col].std()**2
                        ) / (
                            len(agent_dominant[col].dropna()) + 
                            len(df[df["dominant_type"] != agent_type][col].dropna()) - 2
                        ))
        ...
        type_specific[agent_type] = {
            ...
            "significant_predictors": {
                k: v for k, v in advantage_predictors.items() 
                if v["significant"]
            }
        }
    ...
    # 6. Relative Advantage Threshold Analysis
                ...
                try:
                    ...
                    for i in range(1, 10):
                        ...
                        if sign_multiplier * corr > 0:  # Positive correlation
                            above_threshold = df_valid[df_valid[adv_col] >= threshold][score_col].mean()
                            below_threshold = df_valid[df_valid[adv_col] < threshold][score_col].mean()
                        else:  # Negative correlation
                            above_threshold = df_valid[df_valid[adv_col] <= threshold][score_col].mean()
                            below_threshold = df_valid[df_valid[adv_col] > threshold][score_col].mean()
                        
                        dominance_ratio = above_threshold / below_threshold if below_threshold > 0 else float('inf')
                    ...
                    optimal_threshold = max(thresholds, key=lambda x: x["dominance_ratio"])
...
def get_relative_advantage_recommendations(analysis_results: Dict[str, Any]) -> Dict[str, Any]:
    """
    Generate recommendations based on relative advantage analysis.
    ...
    if "agent_type_specific_analysis" not in analysis_results:
        logging.warning("No agent-specific analysis available for generating recommendations")
        return recommendations
    ...
    for agent_type in ["system", "independent", "control"]:
        ...
        if "top_predictors" in agent_analysis:
            for adv_col, data in list(agent_analysis["top_predictors"].items())[:3]:
                ...
                for cat in ["resource_acquisition", "reproduction", "survival", 
                            "population_growth", "combat", "initial_positioning"]:
                    if cat in adv_col:
                        category = cat
                        break
                ...
                types_involved = []
                if "_vs_" in adv_col:
                    pair = adv_col.split('_vs_')[0] + "_vs_" + adv_col.split('_vs_')[1].split('_')[0]
                    types_involved = pair.split('_vs_')
                
                # Format a human-readable recommendation
                favors_direction = "advantage over" if data["dominant_avg"] > 0 else "disadvantage against"
                other_type = types_involved[1] if types_involved[0] == agent_type else types_involved[0]
        ...
        if "advantage_threshold_analysis" in analysis_results and \
           agent_type in analysis_results["advantage_threshold_analysis"]:
            for adv_col, data in analysis_results["advantage_threshold_analysis"][agent_type].items():
        ...
        if "advantage_timing_analysis" in analysis_results and \
           agent_type in analysis_results["advantage_timing_analysis"]:
            ...
            for phase, advantages in phase_data.items():
                if advantages:
                    strengths = [
                        abs(data["average_value"]) 
                        for data in advantages.values()
                    ]
                    if strengths:
            ...
            for i, (phase, strength) in enumerate(sorted_phases):
                phase_name = "early" if phase == "early" else ("middle" if phase == "mid" else "late")
        ...
        if "advantage_category_importance" in analysis_results and \
           "by_category" in analysis_results["advantage_category_importance"]:
            ...
            for category, agent_data in category_data.items():
                if agent_type in agent_data:
                    agent_category_strengths[category] = agent_data[agent_type]["average_relevance"]
            
            # Sort categories by importance for this agent type
            sorted_categories = sorted(
                agent_category_strengths.items(),
                key=lambda x: x[1],
                reverse=True
            )
