...
def compute_learning_efficiency_metrics(df: pd.DataFrame) -> Dict[str, float]:
    ...
    if len(df) > 20:
        ...
        # High convergence rate when halves are similar (low difference)
        reward_range = df["reward"].max() - df["reward"].min()
        if reward_range > 0:
            convergence_rate = 1 - abs(first_half - second_half) / reward_range
        else:
            convergence_rate = 1.0  # Perfect convergence when no variation
    else:
        convergence_rate = 0.0
