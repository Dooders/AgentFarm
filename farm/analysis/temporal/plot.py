...
def plot_temporal_patterns(patterns_df: pd.DataFrame, ctx: AnalysisContext, **kwargs) -> None:
    ...
    for action_type in patterns_df["action_type"].unique():
        ...
        ax1.plot(action_data["time_period"], action_data["action_count"], label=action_type, marker="o", alpha=0.7)
    ...
    for action_type in patterns_df["action_type"].unique():
        action_data = patterns_df[patterns_df["action_type"] == action_type]
        ax2.plot(action_data["time_period"], action_data["avg_reward"], label=action_type, marker="s", alpha=0.7)
...
def plot_rolling_averages(patterns_df: pd.DataFrame, ctx: AnalysisContext, **kwargs) -> None:
    ...
    for action_type in patterns_df["action_type"].unique():
        ...
        if "rolling_avg_reward" in action_data.columns:
            ax1.plot(action_data["time_period"], action_data["rolling_avg_reward"], label=action_type, linewidth=2)
    ...
    for action_type in patterns_df["action_type"].unique():
        ...
        if "rolling_action_count" in action_data.columns:
            ax2.plot(action_data["time_period"], action_data["rolling_action_count"], label=action_type, linewidth=2)
...
def plot_event_segmentation(
    segmentation_data: Dict[str, Any], event_steps: List[int], ctx: AnalysisContext, **kwargs
) -> None:
...
def plot_temporal_efficiency(efficiency_data, ctx: AnalysisContext, **kwargs) -> None:
    """Plot temporal efficiency metrics.

    Args:
        efficiency_data: Dictionary with efficiency metrics or DataFrame
        ctx: Analysis context
        **kwargs: Plot options
    """
    # Handle both DataFrame and dict inputs
    if isinstance(efficiency_data, pd.DataFrame):
        if efficiency_data.empty:
            ctx.logger.warning("No efficiency data for plotting")
            return
        # Convert DataFrame to dict for plotting
        efficiency_dict = efficiency_data.to_dict("index")
        if not efficiency_dict:
            ctx.logger.warning("No efficiency data for plotting")
            return
    else:
        if not efficiency_data:
            ctx.logger.warning("No efficiency data for plotting")
            return
        efficiency_dict = efficiency_data

    ctx.logger.info("Creating temporal efficiency plot...")

    fig, ax = plt.subplots(figsize=(10, 6))

    metrics = list(efficiency_dict.keys())
    values = list(efficiency_dict.values())
    ...
    for bar, value in zip(bars, values):
        ...
        ax.text(bar.get_x() + bar.get_width() / 2.0, height + 0.01, f"{value:.3f}", ha="center", va="bottom")
...
def plot_reward_trends(patterns_df: pd.DataFrame, ctx: AnalysisContext, **kwargs) -> None:
    ...
    for action_type in patterns_df["action_type"].unique():
        ...
        ax.plot(
            action_data["time_period"], action_data["avg_reward"], label=action_type, marker="o", linewidth=2, alpha=0.8
        )
