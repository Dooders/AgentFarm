...
def process_temporal_data(experiment_path: Path, use_database: bool = True, **kwargs) -> pd.DataFrame:
...
def process_time_series_data(experiment_path: Path, time_period_size: int = 100, **kwargs) -> pd.DataFrame:
    ...
    time_series = (
        df.groupby(["time_period", "action_type"])
        .agg({"reward": ["count", "mean", "sum"], "agent_id": "nunique", "step": "min"})
        .reset_index()
    )
...
def process_event_segmentation_data(
    experiment_path: Path, event_steps: Optional[List[int]] = None, **kwargs
...
def extract_temporal_patterns(
    experiment_path_or_df, time_period_size: int = 100, rolling_window_size: int = 10, **kwargs
) -> pd.DataFrame:
    """Extract temporal patterns from action data.

    Args:
        experiment_path_or_df: Path to experiment directory or DataFrame with action data
    ...
    """
    # Handle both Path and DataFrame inputs
    if isinstance(experiment_path_or_df, pd.DataFrame):
        # Direct DataFrame input - process it directly
        df = experiment_path_or_df.copy()

        if df.empty:
            return pd.DataFrame()

        # Add time period column if not present
        if "time_period" not in df.columns and "step" in df.columns:
            df["time_period"] = df["step"] // time_period_size

        # If we don't have aggregated data, aggregate it
        if "action_count" not in df.columns or "avg_reward" not in df.columns:
                    ...
                    .agg(
                        {
                            ...
                            "agent_id": "nunique" if "agent_id" in df.columns else lambda x: 1,
                            "step": "min" if "step" in df.columns else lambda x: 0,
                        }
                    )
                    .reset_index()
                )
        ...
        # Path input - use existing logic
        time_series = process_time_series_data(experiment_path_or_df, time_period_size=time_period_size)
    ...
    for action_type in time_series["action_type"].unique():
        ...
        action_data["rolling_avg_reward"] = (
            action_data["avg_reward"].rolling(window=min(rolling_window_size, len(action_data)), center=True).mean()
        )
        action_data["rolling_action_count"] = (
            action_data["action_count"].rolling(window=min(rolling_window_size, len(action_data)), center=True).mean()
        )
