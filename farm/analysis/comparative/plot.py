        ...
        if i < 4:
            ...
            if "simulation_id" in df.columns:
                try:
                    ...
                    # Fallback for mocked environments or compatibility issues
                    simulation_ids = df["simulation_id"].unique()
                    values = [df[df["simulation_id"] == sim_id][col].values for sim_id in simulation_ids]
                    ax.boxplot(values, labels=simulation_ids)
            else:
    ...
    for i, param in enumerate(unique_params):
        ...
        if "simulation_id" in param_data.columns and "parameter_value" in param_data.columns:
            try:
                ...
                # Fallback for mocked environments or compatibility issues
                simulation_ids = param_data["simulation_id"].unique()
                values = [
                    param_data[param_data["simulation_id"] == sim_id]["parameter_value"].iloc[0]
                    for sim_id in simulation_ids
                ]
                ax.bar(simulation_ids, values)
...
def plot_performance_comparison(df: pd.DataFrame, ctx: AnalysisContext, **kwargs) -> None:
    ...
    performance_cols = [
        col for col in df.columns if any(term in col.lower() for term in ["reward", "performance", "score", "fitness"])
    ]
...
def plot_comparative_analysis(df: pd.DataFrame, ctx: AnalysisContext, **kwargs) -> None:
    ...
    performance_cols = [
        ...
        if any(term in col.lower() for term in ["performance", "score", "reward", "fitness", "efficiency"])
    ]
    ...
    if performance_cols and "experiment" in df.columns:
        ...
        for i, exp in enumerate(experiments):
            ...
            ax2.bar(
                [i + j * 0.2 for j in range(len(performance_cols))],
                means,
                width=0.15,
                label=[f"{exp}-{col}" for col in performance_cols],
            )
