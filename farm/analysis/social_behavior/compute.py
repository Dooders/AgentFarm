...
def compute_social_network_metrics(session: Session) -> Dict[str, Any]:
    ...
    for i, agent_id in enumerate(agent_id_list):
        ...
        out_degree = np.sum(adjacency_matrix[i, :] > 0)
        # In-degree: number of agents that interacted with this agent
        in_degree = np.sum(adjacency_matrix[:, i] > 0)
...
def compute_resource_sharing_metrics(session: Session) -> Dict[str, Any]:
    ...
    share_actions = session.query(
        ...
        AgentModel, AgentModel.agent_id == ActionModel.agent_id
    ).filter(
    ...
    agent_types = {
        a.agent_id: a.agent_type for a in 
        session.query(AgentModel.agent_id, AgentModel.agent_type).all()
    }
    ...
    for action in share_actions:
        ...
        results["sharing_matrix"][action.initiator_type][target_type] += resources_shared
    ...
    altruistic_actions = session.query(
        ActionModel.agent_id,
        ActionModel.resources_before,
...
def compute_spatial_clustering(session: Session, step: Optional[int] = None) -> Dict[str, Any]:
    ...
    agent_positions = session.query(
        AgentStateModel.agent_id,
        AgentStateModel.position_x,
        AgentStateModel.position_y,
        AgentModel.agent_type
    ).join(
        AgentModel, AgentModel.agent_id == AgentStateModel.agent_id
    ).filter(
        AgentStateModel.step_number == step
    ).all()
...
def compute_cooperation_competition_metrics(session: Session) -> Dict[str, Any]:
    ...
    cooperation_actions = session.query(
        ActionModel.agent_id,
        ActionModel.action_target_id,
        ActionModel.action_type,
        ActionModel.step_number,
    ...
    competition_actions = session.query(
        ActionModel.agent_id,
        ActionModel.action_target_id,
        ActionModel.action_type,
        ActionModel.step_number,
    ...
    for action in cooperation_actions:
        ...
        results["cooperation"]["with_agent_type"][action.initiator_type][target_type] += 1
    ...
    for action in competition_actions:
        ...
        results["competition"]["with_agent_type"][action.initiator_type][target_type] += 1
    ...
    for agent_type in set(results["cooperation"]["by_agent_type"].keys()) | set(results["competition"]["by_agent_type"].keys()):
...
def compute_reproduction_social_patterns(session: Session) -> Dict[str, Any]:
    ...
    reproduction_events = session.query(
        ...
        AgentModel, AgentModel.agent_id == ReproductionEventModel.parent_id
    ).filter(
