...
def aggregate_social_behavior_results(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for result in results:
        ...
        if "resource_sharing" in metrics and "error" not in metrics["resource_sharing"]:
            ...
            aggregated["metrics"]["resource_sharing"]["total_sharing_actions"].append(
                rs.get("total_sharing_actions", 0)
            )
            aggregated["metrics"]["resource_sharing"]["total_resources_shared"].append(
                rs.get("total_resources_shared", 0)
            )
            aggregated["metrics"]["resource_sharing"]["avg_resources_per_share"].append(
                rs.get("avg_resources_per_share", 0)
            )
    ...
    aggregated["averages"] = {
        "social_network": {
            "network_density": (
                np.mean(aggregated["metrics"]["social_network"]["network_density"])
                if aggregated["metrics"]["social_network"]["network_density"]
                else 0
            ),
            "total_interactions": (
                np.mean(aggregated["metrics"]["social_network"]["total_interactions"])
                if aggregated["metrics"]["social_network"]["total_interactions"]
                else 0
            ),
            "by_agent_type": {},
        },
        ...
        "spatial_clustering": {
            "total_clusters": (
                np.mean(aggregated["metrics"]["spatial_clustering"]["total_clusters"])
                if aggregated["metrics"]["spatial_clustering"]["total_clusters"]
                else 0
            ),
            "clustering_ratio": (
                np.mean(aggregated["metrics"]["spatial_clustering"]["clustering_ratio"])
                if aggregated["metrics"]["spatial_clustering"]["clustering_ratio"]
                else 0
            ),
            "by_agent_type": {},
        },
        "reproduction_patterns": {
            "total_events": (
                np.mean(aggregated["metrics"]["reproduction_patterns"]["total_events"])
                if aggregated["metrics"]["reproduction_patterns"]["total_events"]
                else 0
            ),
            "isolation_pct": (
                np.mean(aggregated["metrics"]["reproduction_patterns"]["isolation_pct"])
                if aggregated["metrics"]["reproduction_patterns"]["isolation_pct"]
                else 0
            ),
            "by_agent_type": {},
        },
