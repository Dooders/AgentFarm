    ...
    try:
        ...
        for agent_type, data in stats.items():
            if data["count"] > 0:
                ...
                result[f"{agent_type}_avg_survival"] = data["total_survival"] / data["count"]
                if data["dead"] > 0:
...
def get_initial_positions_and_resources(sim_session, config):
    ...
    initial_agents = sim_session.query(AgentModel).filter(AgentModel.birth_time == 0).all()

    # Get the initial resources (step_number = 0)
    initial_resources = sim_session.query(ResourceModel).filter(ResourceModel.step_number == 0).all()
    ...
    # Extract positions
    agent_positions = {agent.agent_type.lower(): (agent.position_x, agent.position_y) for agent in initial_agents}
    ...
    for agent_type, pos in agent_positions.items():
        ...
        distances = [euclidean(pos, (r_pos[0], r_pos[1])) for r_pos in resource_positions]
        ...
        if distances:
            ...
            result[f"{agent_type}_avg_resource_dist"] = float(sum(distances) / len(distances))

            # Calculate weighted distance (by resource amount)
            weighted_distances = [
                distances[i] * (1 / (resource_positions[i][2] + 1))  # Add 1 to avoid division by zero
                for i in range(len(distances))
            ]
            result[f"{agent_type}_weighted_resource_dist"] = float(sum(weighted_distances) / len(weighted_distances))
            ...
            resource_amount_in_range = sum(
                resource_positions[i][2] for i in range(len(distances)) if distances[i] <= gathering_range
            )
    ...
    for i, type1 in enumerate(agent_types):
        for type2 in agent_types[i + 1 :]:
            ...
            if key1 in result and key2 in result:
                result[f"{type1}_vs_{type2}_nearest_resource_advantage"] = result[key2] - result[key1]

            # Difference in resources in range
            key1 = f"{type1}_resources_in_range"
            key2 = f"{type2}_resources_in_range"
            if key1 in result and key2 in result:
                result[f"{type1}_vs_{type2}_resources_in_range_advantage"] = result[key1] - result[key2]

            # Difference in resource amount in range
            key1 = f"{type1}_resource_amount_in_range"
            key2 = f"{type2}_resource_amount_in_range"
            if key1 in result and key2 in result:
                result[f"{type1}_vs_{type2}_resource_amount_advantage"] = result[key1] - result[key2]
...
def get_reproduction_stats(sim_session):
    ...
    try:
        ...
        offspring_agents = sim_session.query(AgentModel).filter(AgentModel.birth_time > 0).all()

        # Get failed reproduction attempts from agent_actions
        reproduce_actions = sim_session.query(ActionModel).filter(ActionModel.action_type == "reproduce").all()
        ...
        logger.info(f"Found {len(offspring_agents)} successful reproductions and {len(failed_actions)} failed attempts")
        ...
        try:
            ...
            logger.info(f"Original agent types in database: {', '.join(agent_types_found)}")
            ...
            try:
                ...
                if parent_type not in stats:
                    if parent_type not in unknown_agent_types:
                        ...
                        logger.warning(f"Unknown agent type: {parent_type} for agent {parent_id}")
                    continue
                ...
                try:
                    ...
                    parent_state_after = (
                        sim_session.query(AgentStateModel)
                        .filter(
                            AgentStateModel.agent_id == parent_id, AgentStateModel.step_number == offspring.birth_time
                        )
                        .first()
                    )

                    if parent_state_before and parent_state_after:
                        resources_spent = max(0, parent_state_before.resource_level - parent_state_after.resource_level)
        ...
        if missing_resource_data > 0:
            logger.warning(f"Missing resource data for {missing_resource_data} reproduction events")

        if unknown_agent_types:
            logger.warning(f"Found unknown agent types: {', '.join(unknown_agent_types)}")
        ...
        for agent_type, data in stats.items():
            if data["attempts"] > 0:
                ...
                result[f"{agent_type}_reproduction_success_rate"] = data["successes"] / data["attempts"]

                # Calculate resource metrics if we have resource data
                if data["resources_spent"] > 0:
                    result[f"{agent_type}_avg_resources_per_reproduction"] = data["resources_spent"] / data["attempts"]
                ...
                if data["first_reproduction_time"] != float("inf"):
                    result[f"{agent_type}_first_reproduction_time"] = data["first_reproduction_time"]
                else:
        ...
        for i, type1 in enumerate(agent_types):
            for type2 in agent_types[i + 1 :]:
                ...
                if key1 in result and key2 in result:
                    result[f"{type1}_vs_{type2}_reproduction_rate_advantage"] = result[key1] - result[key2]
                ...
                if key1 in result and key2 in result and result[key1] > 0 and result[key2] > 0:
                    result[f"{type1}_vs_{type2}_first_reproduction_advantage"] = result[key2] - result[key1]

                # Difference in reproduction efficiency
                key1 = f"{type1}_reproduction_efficiency"
                key2 = f"{type2}_reproduction_efficiency"
                if key1 in result and key2 in result:
                    result[f"{type1}_vs_{type2}_reproduction_efficiency_advantage"] = result[key1] - result[key2]
        ...
        if result:
            ...
            for agent_type in agent_types:
                ...
                if attempts_key in result:
                    ...
                    logger.info(f"  {agent_type}: {attempts} attempts, {success_rate:.1f}% success rate")
