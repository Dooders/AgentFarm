...
# ============================================================================
# Backward Compatibility: Module-level functions that delegate to class
# ============================================================================

# Global instance for backward compatibility
_default_computer = DominanceComputer()


def compute_population_dominance(sim_session) -> Optional[str]:
    """
    Backward compatibility wrapper for compute_population_dominance.
    
    Delegates to the default DominanceComputer instance.
    """
    return _default_computer.compute_population_dominance(sim_session)


def compute_survival_dominance(sim_session) -> Optional[str]:
    """
    Backward compatibility wrapper for compute_survival_dominance.
    
    Delegates to the default DominanceComputer instance.
    """
    return _default_computer.compute_survival_dominance(sim_session)


def compute_dominance_switches(sim_session) -> Optional[Dict[str, Any]]:
    """
    Backward compatibility wrapper for compute_dominance_switches.
    
    Delegates to the default DominanceComputer instance.
    """
    return _default_computer.compute_dominance_switches(sim_session)


def compute_comprehensive_dominance(sim_session) -> Optional[Dict[str, Any]]:
    """
    Backward compatibility wrapper for compute_comprehensive_dominance.
    
    Delegates to the default DominanceComputer instance.
    """
    return _default_computer.compute_comprehensive_dominance(sim_session)


def compute_dominance_switch_factors(df: pd.DataFrame) -> Optional[Dict[str, Any]]:
    """
    Backward compatibility wrapper for compute_dominance_switch_factors.
    
    Delegates to the default DominanceComputer instance.
    """
    return _default_computer.compute_dominance_switch_factors(df)


def aggregate_reproduction_analysis_results(
    df: pd.DataFrame, numeric_repro_cols: List[str]
) -> Dict[str, Any]:
    """
    Backward compatibility wrapper for aggregate_reproduction_analysis_results.
    
    Delegates to the default DominanceComputer instance.
    """
    return _default_computer.aggregate_reproduction_analysis_results(df, numeric_repro_cols)
