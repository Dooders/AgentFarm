...
def plot_combat_overview(metrics_df: pd.DataFrame, ctx: AnalysisContext, **kwargs) -> None:
    ...
    ax1.plot(metrics_df["step"], metrics_df["combat_encounters"], label="Combat Encounters", linewidth=2, color="red")
    ...
    ax2.plot(
        metrics_df["step"], metrics_df["successful_attacks"], label="Successful Attacks", linewidth=2, color="orange"
    )
...
def plot_combat_success_rate(metrics_df: pd.DataFrame, ctx: AnalysisContext, **kwargs) -> None:
    ...
    ax.plot(metrics_df["step"], success_rate * 100, linewidth=2, color="green", label="Success Rate")
...
def plot_agent_combat_performance(agent_combat_df: pd.DataFrame, ctx: AnalysisContext, **kwargs) -> None:
    ...
    # Calculate average damage if not present
    if "avg_damage" not in sorted_df.columns:
        sorted_df = sorted_df.copy()
        sorted_df["avg_damage"] = sorted_df["total_damage"] / sorted_df["total_attacks"].replace(0, 1)
    ...
    ax1.set_yticklabels([f"Agent {id}" for id in sorted_df["agent_id"]])
    ...
    ax2.set_yticklabels([f"Agent {id}" for id in sorted_df["agent_id"]])
    ...
    ax3.set_yticklabels([f"Agent {id}" for id in sorted_df["agent_id"]])
    ...
    ax4.set_yticklabels([f"Agent {id}" for id in sorted_df["agent_id"]])
...
def plot_combat_efficiency(efficiency_data: Dict[str, float], ctx: AnalysisContext, **kwargs) -> None:
    ...
    Args:
        ...
        **kwargs: Plot options
    """
    if not efficiency_data or len(efficiency_data) == 0:
    ...
    for bar, value in zip(bars, values):
        ...
        ax.text(
            bar.get_x() + bar.get_width() / 2.0, height + max(values) * 0.01, f"{value:.3f}", ha="center", va="bottom"
        )
...
def plot_combat_temporal_patterns(patterns_data: Dict[str, Any], ctx: AnalysisContext, **kwargs) -> None:
    ...
    Args:
        ...
        **kwargs: Plot options
    """
    if not patterns_data or len(patterns_data) == 0:
