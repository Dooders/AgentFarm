...
class SocialDynamicsAnalyzer:
    ...
    def __init__(
        self, experiment_path: str, output_dir: str = "social_analysis_results"
    ):
        """
    ...
    def _collect_competition_metrics(
        self, session: sqlalchemy.orm.Session, iteration: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if final_step_data:
                ...
                if hasattr(final_step_data, "combat_encounters"):
                    metrics["total_combat_encounters"] = (
                ...
                if hasattr(final_step_data, "successful_attacks"):
                    metrics["total_successful_attacks"] = (
            ...
            attack_action_type = find_action_type(
                session, ["attack", "ATTACK", "combat", "fight", "Attack"]
            )
            ...
            if attack_action_type:
                ...
                for agent_type in agent_types:
                    ...
                    metrics[f"{agent_type}_successful_attacks"] = (
                        type_successful_attacks
                    )
                ...
                self.logger.warning(
                    f"Could not find attack action type for iteration {iteration}"
                )
                # Set default values
                agent_types = ["SystemAgent", "IndependentAgent", "ControlAgent"]
                for agent_type in agent_types:
    ...
    def _collect_cooperation_metrics(
        self, session: sqlalchemy.orm.Session, iteration: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if final_step_data and hasattr(final_step_data, "resources_shared"):
                ...
                metrics["avg_resources_shared_per_step"] = (
                    final_step_data.resources_shared / final_step
                    if final_step > 0
                    else 0
                )

            # Find and analyze sharing actions
            share_action_type = find_action_type(
                session,
                [
                    "share",
                    "SHARE",
            ...
            if share_action_type:
                ...
                share_actions = (
                    session.query(ActionModel)
                    .filter(ActionModel.action_type == share_action_type)
                    .count()
                )

                metrics["share_actions"] = share_actions
                metrics["share_actions_per_step"] = (
                    share_actions / final_step if final_step > 0 else 0
                )
                metrics["share_actions_per_agent"] = (
                    share_actions / total_agents if total_agents > 0 else 0
                )
                ...
                for agent_type in agent_types:
                    ...
                    agent_count = metrics.get(f"{agent_type}_agents", 0)
                    if agent_count > 0:
                        metrics[f"{agent_type}_share_actions_per_agent"] = (
                            type_share_actions / agent_count
                        )
                    else:
                        metrics[f"{agent_type}_share_actions_per_agent"] = 0
            else:
                self.logger.warning(
                    f"Could not find share action type for iteration {iteration}"
                )
                # Set default values
                agent_types = ["SystemAgent", "IndependentAgent", "ControlAgent"]
                for agent_type in agent_types:
            ...
            self.logger.error(
                f"Error collecting cooperation metrics for iteration {iteration}: {e}"
            )

        return metrics

    def _analyze_combat_health_incidents(
        self,
    ...
    def _analyze_reproduction_cooperation(
        self,
        session: sqlalchemy.orm.Session,
        metrics: Dict[str, Any],
        ...
        try:
            ...
            metrics["reproduction_success_rate"] = (
                successful_reproductions / reproduction_events
                if reproduction_events > 0
                else 0
            )

            # Calculate average resource transfer in successful reproductions
            if successful_reproductions > 0:
                avg_resource_transfer_result = (
                    session.query(
                        func.avg(
                ...
                metrics["avg_reproduction_resource_transfer"] = (
                    avg_resource_transfer_result or 0
                )
            ...
            for agent_type in agent_types:
                type_reproductions = (
                    ...
                    .join(
                        AgentModel,
                        ReproductionEventModel.parent_id == AgentModel.agent_id,
                    )
                ...
                metrics[f"{agent_type}_reproduction_success_rate"] = (
                    type_successful / type_reproductions
                    if type_reproductions > 0
                    else 0
                )
    ...
    def analyze_social_dynamics(self) -> Dict[str, Any]:
        ...
        results = {
            "competition_analysis": {
                "data": (
                    competition_df.to_dict("records")
                    if not competition_df.empty
                    else []
                ),
                "summary": self._calculate_social_summary(
                    competition_df, "competition"
                ),
            },
            "cooperation_analysis": {
                "data": (
                    cooperation_df.to_dict("records")
                    if not cooperation_df.empty
                    else []
                ),
                "summary": self._calculate_social_summary(
                    cooperation_df, "cooperation"
                ),
            },
            "social_balance": self._calculate_social_balance(
                competition_df, cooperation_df
            ),
        }

        return results

    def _calculate_social_summary(
    ...
    def _calculate_social_balance(
        self, competition_df: pd.DataFrame, cooperation_df: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        balance_ratio = (
            avg_cooperation / (avg_competition + avg_cooperation)
            if (avg_competition + avg_cooperation) > 0
            else 0
        )
        ...
        return {
            ...
            "social_climate": (
                ...
                else "competitive" if balance_ratio < 0.4 else "balanced"
            ),
        }
    ...
    def _create_competition_visualizations(self, df: pd.DataFrame) -> None:
        ...
        if "competition_index" in df.columns:
            ...
            save_figure(
                fig, os.path.join(self.output_dir, "competition_index_distribution.png")
            )
        ...
        if all(col in df.columns for col in attack_cols):
            fig, ax = plt.subplots(figsize=(10, 6))
            attack_data = [df[col].values.tolist() for col in attack_cols]
            create_box_plot(
                ax,
                attack_data,
                agent_types,
            ...
            save_figure(
                fig, os.path.join(self.output_dir, "attack_actions_by_type.png")
            )
    ...
    def _create_cooperation_visualizations(self, df: pd.DataFrame) -> None:
        ...
        if "cooperation_index" in df.columns:
            ...
            save_figure(
                fig, os.path.join(self.output_dir, "cooperation_index_distribution.png")
            )

        # Sharing actions by agent type
        agent_types = ["SystemAgent", "IndependentAgent", "ControlAgent"]
        share_cols = [
            f"{agent_type}_share_actions_per_agent" for agent_type in agent_types
        ]

        if all(col in df.columns for col in share_cols):
            fig, ax = plt.subplots(figsize=(10, 6))
            share_data = [df[col].values.tolist() for col in share_cols]
            create_box_plot(
                ax,
                share_data,
                agent_types,
    ...
    def _create_social_balance_visualizations(
        self, competition_df: pd.DataFrame, cooperation_df: pd.DataFrame
    ) -> None:
        ...
        if (
            "competition_index" in competition_df.columns
            and "cooperation_index" in cooperation_df.columns
        ):
            comp_means = [
                competition_df[f"{agent_type}_attack_actions_per_agent"].mean()
                for agent_type in ["SystemAgent", "IndependentAgent", "ControlAgent"]
            ]
            coop_means = [
                cooperation_df[f"{agent_type}_share_actions_per_agent"].mean()
                for agent_type in ["SystemAgent", "IndependentAgent", "ControlAgent"]
            ]
...
def main():
    ...
    results = analyze_social_dynamics(
        experiment_path, "social_dynamics_analysis", create_visualizations=True
    )
    ...
    if (
        "competition_analysis" in results
        and "summary" in results["competition_analysis"]
    ):
        comp_summary = results["competition_analysis"]["summary"]
        print(f"Mean competition score: {comp_summary.get('mean_score', 0):.3f}")
        print(f"Number of simulations: {comp_summary.get('n_simulations', 0)}")

    if (
        "cooperation_analysis" in results
        and "summary" in results["cooperation_analysis"]
    ):
        coop_summary = results["cooperation_analysis"]["summary"]
        print(f"Mean cooperation score: {coop_summary.get('mean_score', 0):.3f}")
