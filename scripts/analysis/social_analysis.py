...
Social analysis orchestration script using the unified analysis framework.

This script provides a simple interface to run social behavior analysis
using the farm.analysis.social_behavior module through the AnalysisService.
...
2. Run comprehensive social behavior analysis
...
def main():
    """Main function to run social behavior analysis."""
    start_time = datetime.now()
    print("Starting social behavior analysis script...")

    try:
        # Create output directory
        social_output_path = os.path.join(OUTPUT_PATH, "social_behavior")

        # Clear the output directory if it exists
        if os.path.exists(social_output_path):
            print(f"Clearing existing output directory: {social_output_path}")
            if not safe_remove_directory(social_output_path):
                # Create timestamped alternative
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                social_output_path = os.path.join(OUTPUT_PATH, f"social_behavior_{timestamp}")
                print(f"Using alternative directory: {social_output_path}")

        # Create the directory
        os.makedirs(social_output_path, exist_ok=True)

        # Set up logging
        log_file = setup_logging(social_output_path)
        print(f"Saving results to {social_output_path}")
        ...
        print("Running social behavior analysis...")
        request = AnalysisRequest(
            module_name="social_behavior",
            experiment_path=Path(experiment_path),
            output_path=Path(social_output_path),
        ...
        print(f"All results saved to: {social_output_path}")
