...
describe('Zod Schema Validation System - Acceptance Criteria Tests', () => {
  ...
  describe('✅ All simulation config fields have Zod validation', () => {
    ...
    it('should validate enum fields correctly', async () => {
      ...
      expect(result.success).toBe(false);
      expect(
        result.errors.some((e) => e.message.includes('floor, round, or ceil'))
      ).toBe(true);
    });
  });
  ...
  describe('✅ Nested object validation works correctly', () => {
    ...
    it('should validate nested visualization config', async () => {
      ...
      expect(result.success).toBe(false);
      expect(
        result.errors.some((e) => e.path.includes('visualization.canvas_width'))
      ).toBe(true);
      expect(
  ...
  describe('✅ Custom validation rules are implemented', () => {
    it('should enforce agent type ratios sum to 1.0', async () => {
      ...
      expect(result.success).toBe(false);
      expect(
        result.errors.some((e) => e.message.includes('sum to exactly 1.0'))
      ).toBe(true);
    });
    ...
    it('should enforce epsilon hierarchy', async () => {
      ...
      expect(
        result.errors.some((e) =>
          e.message.includes(
            'Epsilon minimum must be less than or equal to epsilon start'
          )
        )
      ).toBe(true);
    });
  ...
  describe('✅ Error messages are properly formatted', () => {
    ...
    it('should provide format-specific error messages', async () => {
      ...
      expect(result.success).toBe(false);
      expect(
        result.errors.some((e) => e.message.includes('hex color code'))
      ).toBe(true);
    });
  ...
  describe('✅ TypeScript integration works seamlessly', () => {
    it('should provide full type safety with Zod schemas', () => {
      ...
      const config: Parameters<typeof SimulationConfigSchema.parse>[0] =
        validConfig;
      ...
      expect(typeof parsed.agent_parameters.SystemAgent.learning_rate).toBe(
        'number'
      );
    ...
    it('should work with safeValidateConfig returning typed data', () => {
      ...
      if (result.success && result.data) {
        ...
        expect(typedConfig.agent_parameters.SystemAgent.memory_size).toBe(
          10000
        );
  ...
  describe('Field-level validation', () => {
    ...
    it('should validate nested field paths', () => {
      const validNestedErrors = validateField(
        'agent_parameters.SystemAgent.learning_rate',
        0.001
      );
      expect(validNestedErrors).toHaveLength(0);

      const invalidNestedErrors = validateField(
        'agent_parameters.SystemAgent.learning_rate',
        2.0
      );
      expect(invalidNestedErrors.length).toBeGreaterThan(0);
      expect(invalidNestedErrors[0].path).toBe(
        'agent_parameters.SystemAgent.learning_rate'
      );
    });
  });
