...
def generate_resource_positions(pattern, num_resources=20, grid_size=(100, 100)):
    ...
    if pattern == 'random':
        ...
        corners = [(10, 10), (10, height-10), (width-10, 10), (width-10, height-10)]
...
def visualize_experiment_setup(resource_positions, agent_positions, title, filename):
    ...
    for agent_type, (x, y) in agent_positions.items():
        ax.scatter(x, y, s=100, c=agent_colors[agent_type], edgecolors='black', label=agent_type)
...
def create_experiment_config(resource_pattern, agent_scenario, experiment_id):
    ...
    config['initial_positions'] = {
        'resources': [{'position_x': x, 'position_y': y, 'amount': amount} for x, y, amount in resource_positions],
        'agents': {agent_type: {'position_x': pos[0], 'position_y': pos[1]} for agent_type, pos in agent_positions.items()}
    }
...
def main():
    ...
    agent_scenarios = ['equal_distance', 'one_advantaged', 'one_disadvantaged', 'gradient_advantage', 'random']
    ...
    for pattern in resource_patterns:
        for scenario in agent_scenarios:
            for rep in range(repetitions):
                ...
                # Save config to file
                config_filename = f"controlled_experiments/configs/experiment_{experiment_id}.json"
                ...
                visualize_experiment_setup(resource_positions, agent_positions, title, viz_filename)
