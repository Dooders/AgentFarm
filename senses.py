...
class LatentSpace:
    def __init__(self, dimension=768):
        ...
        """
        self.index = faiss.IndexFlatL2(dimension)  # Vector database for storing embeddings
    ...
    def search(self, embedding, top_k=5):
        ...
        results = [(self.metadata[i], distances[0][idx]) for idx, i in enumerate(indices[0])]
        return results
...
class Senses:
    ...
    def generate_embedding(self, image_path):
        ...
        with torch.no_grad():
            ...
            embedding = outputs.last_hidden_state[:, 0, :].cpu().numpy()  # Extract CLS token embedding
        return embedding
