...
from farm.database.models import AgentModel, SimulationStepModel, ResourceModel, ReproductionEventModel
...
def get_initial_positions_and_resources(sim_session, config):
    ...
    for agent_type, pos in agent_positions.items():
        ...
        if distances:
            ...
            result[f"{agent_type}_weighted_resource_dist"] = sum(weighted_distances) / len(weighted_distances)
            ...
            resource_amount_in_range = sum(
                resource_positions[i][2] for i in range(len(distances)) 
...
def get_reproduction_stats(sim_session):
    ...
    agents = {agent.agent_id: agent.agent_type.lower() for agent in sim_session.query(AgentModel).all()}
    
    # Initialize counters
    stats = {
        "system": {"attempts": 0, "successes": 0, "failures": 0, "first_reproduction_time": float('inf')},
        "independent": {"attempts": 0, "successes": 0, "failures": 0, "first_reproduction_time": float('inf')},
        "control": {"attempts": 0, "successes": 0, "failures": 0, "first_reproduction_time": float('inf')}
    }
    ...
    for event in reproduction_events:
        ...
        if event.success:
            ...
            stats[parent_type]["first_reproduction_time"] = min(
                stats[parent_type]["first_reproduction_time"], 
                event.step_number
            )
        else:
    ...
    for folder in iteration_folders:
        ...
        try:
            ...
            key_params = [
                ...
                "initial_resource_level", "max_population", "starvation_threshold",
                "offspring_cost", "min_reproduction_resources", "offspring_initial_resources",
                "perception_radius", "base_attack_strength", "base_defense_strength",
                "resource_regen_rate", "resource_regen_amount", "max_resource_amount",
                "base_consumption_rate", "max_movement", "gathering_range",
                "max_gather_amount", "territory_range", "starting_health",
                "attack_range", "attack_base_damage", "attack_kill_reward",
                "simulation_steps"
            ]
...
def plot_resource_proximity_vs_dominance(df):
    ...
    """
    resource_metrics = [col for col in df.columns if 'resource' in col and 'dist' in col]
...
def plot_correlation_matrix(df, label_name):
    ...
    for target_class in target_numeric.columns:
        correlations[target_class] = numeric_features.corrwith(target_numeric[target_class])
...
def main():
    ...
    exclude_cols = [
        'iteration', 'population_dominance', 'survival_dominance',
