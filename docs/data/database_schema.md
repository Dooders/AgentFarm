...
| details          | STRING(1024)        | Additional action details (JSON) |
| module_type      | STRING(50)          | Type of learning module (nullable) |
| module_id        | STRING(64)          | Identifier for specific module (nullable) |

**Note**: For actions generated by learning modules, `module_type` and `module_id` identify the learning module. The `details` JSON field contains `action_taken` (integer) and `action_taken_mapped` (string) when available.
