...
# docs/concepts/initial-diversity.md for the full reference.
...
| `coparent_strategy`      | `"nearest_alive_same_type"`    | Either `nearest_alive_same_type` or `random_alive_same_type`. When `allow_cross_type_pollination` is `False` (default), both filter to alive agents of the same `agent_type` with `hyperparameter_chromosome`s; when `True`, all alive agents with `hyperparameter_chromosome`s are eligible, still subject to `coparent_max_radius` if set. |
