...
def extract_time_series(db_path):
    ...
    SELECT step_number, total_agents, system_agents, independent_agents, control_agents,
           ...
           average_agent_age, combat_encounters, successful_attacks, resources_shared
    ...
    if not repro_df.empty:
        ...
        if 'success' in repro_df.columns:
            ...
            repro_counts = successful_repro.groupby(['step_number', 'parent_type']).size().unstack(fill_value=0)
            
            # Rename columns to standardize
            if 'SystemAgent' in repro_counts.columns:
                repro_counts.rename(columns={'SystemAgent': 'system_reproduction'}, inplace=True)
            else:
                repro_counts['system_reproduction'] = 0
                
            if 'IndependentAgent' in repro_counts.columns:
                repro_counts.rename(columns={'IndependentAgent': 'independent_reproduction'}, inplace=True)
            else:
                repro_counts['independent_reproduction'] = 0
                
            if 'ControlAgent' in repro_counts.columns:
                repro_counts.rename(columns={'ControlAgent': 'control_reproduction'}, inplace=True)
            else:
                repro_counts['control_reproduction'] = 0
            
            # Merge reproduction counts with steps data
            steps_df = pd.merge(steps_df, repro_counts.reset_index(), on='step_number', how='left')
            steps_df.fillna({'system_reproduction': 0, 'independent_reproduction': 0, 'control_reproduction': 0}, inplace=True)
...
def calculate_initial_advantages(agents_df, resources_df, gathering_range=30):
    ...
    for _, agent in agents_df.iterrows():
        ...
        if distances:
            ...
            resource_amount_in_range = sum(amount for dist, amount in distances if dist <= gathering_range)
...
def plot_population_time_series(steps_df, initial_advantages, title, filename):
    ...
    ax.plot(steps_df['step_number'], steps_df['system_agents'], 'b-', label='System Agents')
    ax.plot(steps_df['step_number'], steps_df['independent_agents'], 'r-', label='Independent Agents')
    ax.plot(steps_df['step_number'], steps_df['control_agents'], 'orange', label='Control Agents')
    ...
    for agent_type, metrics in initial_advantages.items():
        ...
        # Add text annotation
        ax.text(0.02, y_pos, text, transform=ax.transAxes, color=color, fontsize=10,
...
def plot_reproduction_time_series(steps_df, repro_df, title, filename):
    ...
    ax.plot(steps_df['step_number'], steps_df['system_reproduction'].cumsum(), 'b-', label='System Reproduction')
    ax.plot(steps_df['step_number'], steps_df['independent_reproduction'].cumsum(), 'r-', label='Independent Reproduction')
    ax.plot(steps_df['step_number'], steps_df['control_reproduction'].cumsum(), 'orange', label='Control Reproduction')
    ...
    for agent_type in ['SystemAgent', 'IndependentAgent', 'ControlAgent']:
        ...
        if not type_repro.empty and 'success' in type_repro.columns:
            ...
            if not success_repro.empty:
                ...
                ax.text(first_step + 5, ax.get_ylim()[1] * (0.1 + 0.05 * len(first_repro)), 
                        f'First {agent_type} reproduction', color=color, fontsize=9)
...
def plot_resource_acquisition(steps_df, title, filename):
    ...
    ax1.plot(steps_df['step_number'], steps_df['total_resources'], 'g-', label='Total Resources')
    ax1.set_ylabel('Total Resources')
    ax1.legend(loc='upper right')
    ax1.grid(True, alpha=0.3)
    
    # Plot average agent resources
    ax2.plot(steps_df['step_number'], steps_df['average_agent_resources'], 'b-', label='Avg Agent Resources')
    
    # Add resource sharing if available
    if 'resources_shared' in steps_df.columns:
        # Plot on secondary y-axis
        ax3 = ax2.twinx()
        ax3.plot(steps_df['step_number'], steps_df['resources_shared'], 'r-', label='Resources Shared')
...
def plot_comparative_analysis(simulations_data, title, filename):
    ...
    for i, (sim_id, data) in enumerate(simulations_data.items()):
        ...
        ax.plot(steps_df['step_number'], steps_df['independent_ratio'], 'r-', label='Independent')
        ax.plot(steps_df['step_number'], steps_df['control_ratio'], 'orange', label='Control')
        ...
        for agent_type, metrics in advantages.items():
            ...
            ax.text(0.02, y_pos, text, transform=ax.transAxes, color=color, fontsize=8,
                    bbox=dict(facecolor='white', alpha=0.7, edgecolor='gray'))
...
def main():
    ...
    dominance_groups = {
        ...
        'independent': df[df['population_dominance'] == 'independent']['iteration'].tolist(),
    ...
    for dom_type, iterations in dominance_groups.items():
        if len(iterations) > 3:
            selected_iterations[dom_type] = np.random.choice(iterations, 3, replace=False).tolist()
        else:
    ...
    for dom_type, iterations in selected_iterations.items():
        for iteration in iterations:
            print(f"Analyzing time series for iteration {iteration} (dominance: {dom_type})")
            ...
            initial_advantages = calculate_initial_advantages(initial_agents, initial_resources, gathering_range)
    ...
    for dom_type, iterations in selected_iterations.items():
        ...
        dom_simulations = {it: simulations_data[it] for it in iterations if it in simulations_data}
        
        if dom_simulations:
            title = f"Comparative Analysis - {dom_type.capitalize()} Dominance Simulations"
    ...
    print("Time series analysis complete. Results saved to 'time_series_analysis' directory.")
